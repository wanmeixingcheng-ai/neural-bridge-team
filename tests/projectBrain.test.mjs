...
test("knowledge export manifest summarizes governance preservation risks", () => {
  const manifest = knowledgeBrainExportManifest({
    source_registry:[
      { id:"src-1", review_status:"approved", risk_level:"low", version:1 },
    ],
    knowledge_units:[
      { id:"ku-ok", source_id:"src-1", evidence_ref_ids:["ev-1"], review_status:"approved", risk_level:"high", version:2 },
      { id:"ku-risk", source_id:"", evidence_ref_ids:[], review_status:"candidate", risk_level:"restricted", version:0 },
    ],
    evidence_refs:[
  ...
  assert.deepEqual(manifest.toolValidationRunQualityActions, []);
...
test("source reference search eligibility explains approved search blockers", () => {
  const report = sourceReferenceSearchEligibilityReport([
    { id:"src-ok", title:"OK", review_status:"approved", risk_level:"low", deletion_requested:false },
    { id:"src-high-missing-reviewer", title:"High", review_status:"approved", risk_level:"high", deletion_requested:false, metadata:{} },
    { id:"src-deleted", title:"Deleted", review_status:"approved", risk_level:"low", deletion_requested:true },
    { id:"src-candidate", title:"Candidate", review_status:"candidate", risk_level:"low", deletion_requested:false },
  ]);

  assert.equal(report.find(item => item.source_id === "src-ok").eligible, true);
  assert.deepEqual(report.find(item => item.source_id === "src-high-missing-reviewer").reasons, ["missing_approved_review_metadata"]);
  assert.deepEqual(report.find(item => item.source_id === "src-deleted").reasons, ["deletion_requested"]);
  assert.deepEqual(report.find(item => item.source_id === "src-candidate").reasons, ["source_not_approved", "missing_approved_review_metadata"]);
  assert.deepEqual(filterSourceReferenceSearchEligibilityReport(report, {
    eligible:false,
    reasons:["missing_approved_review_metadata"],
    query:"high",
  }).map(item => item.source_id), ["src-high-missing-reviewer"]);
  assert.deepEqual(filterSourceReferenceSearchEligibilityReport(report, {
    sourceTypes:[""],
    reviewStatuses:["approved"],
    riskLevels:["low"],
    eligible:true,
  }).map(item => item.source_id), ["src-ok"]);
  const actions = sourceReferenceSearchEligibilityActions(report);
  assert.ok(actions.some(item => item.action === "record_expert_reviewer_metadata" && item.sourceIds.includes("src-high-missing-reviewer")));
  assert.ok(actions.some(item => item.action === "honor_source_deletion_or_restore_with_owner_approval" && item.sourceIds.includes("src-deleted")));
  assert.ok(actions.some(item => item.action === "route_source_to_review" && item.sourceIds.includes("src-candidate")));

  const hits = approvedKnowledgeUnitSearchResults({
    query:"hazard",
    sources:[
      { id:"src-ok", review_status:"approved", risk_level:"low", deletion_requested:false },
      { id:"src-high-missing-reviewer", review_status:"approved", risk_level:"high", deletion_requested:false, metadata:{} },
    ],
    units:[
      { id:"ku-ok", source_id:"src-ok", title:"Hazard", content:"hazard approved", review_status:"approved", risk_level:"low", version:1 },
      { id:"ku-blocked", source_id:"src-high-missing-reviewer", title:"Hazard", content:"hazard blocked", review_status:"approved", risk_level:"low", version:1 },
    ],
  });

  assert.deepEqual(hits.map(item => item.id), ["ku-ok"]);
});
...
test("knowledge panel search includes approved knowledge brain records and legacy chunks", () => {
  const hits = knowledgePanelSearchResults({
    query:"hazard",
    documents:[
      { id:"doc-approved", status:"approved", archived:false },
      { id:"doc-candidate", status:"candidate", archived:false },
    ],
    chunks:[
      { id:"chunk-ok", docId:"doc-approved", content:"hazard hazard legacy note" },
      { id:"chunk-candidate", docId:"doc-candidate", content:"hazard candidate note" },
    ],
    sources:[
      { id:"src-approved", review_status:"approved", deletion_requested:false },
      { id:"src-candidate", review_status:"candidate", deletion_requested:false },
...
test("source training eligibility report filters governance blockers", () => {
  const report = sourceTrainingEligibilityReport([
    { id:"ok", title:"MLIT public guide", provider:"MLIT", source_type:"public_manual", review_status:"approved", training_allowed:true, consent_scope:"explicit_opt_in", deletion_requested:false, risk_level:"low" },
    { id:"candidate", title:"Partner draft", provider:"PartnerCo", source_type:"desensitized_case", review_status:"candidate", training_allowed:false, consent_scope:"none", deletion_requested:false, risk_level:"medium" },
    { id:"contract", title:"Contract draft", provider:"Owner", source_type:"contract", review_status:"approved", training_allowed:true, consent_scope:"explicit_opt_in", deletion_requested:false, risk_level:"restricted" },
  ]);

  assert.deepEqual(filterSourceTrainingEligibilityReport(report, { eligible:true }).map(item => item.source_id), ["ok"]);
  assert.deepEqual(filterSourceTrainingEligibilityReport(report, { eligible:false, reasons:["source_not_approved"] }).map(item => item.source_id), ["candidate"]);
  assert.deepEqual(filterSourceTrainingEligibilityReport(report, { sourceTypes:["contract"], riskLevels:["restricted"] }).map(item => item.source_id), ["contract"]);
  assert.deepEqual(filterSourceTrainingEligibilityReport(report, { query:"Partner draft" }).map(item => item.source_id), ["candidate"]);
  assert.deepEqual(filterSourceTrainingEligibilityReport(report, { query:"high_risk_source_type" }).map(item => item.source_id), ["contract"]);
});
...
test("source usage permission actions map blocked scopes to repair work", () => {
  const actions = sourceUsagePermissionActions(sourceUsagePermissionReport([
    { id:"candidate", source_type:"public_manual", review_status:"candidate", risk_level:"low", training_allowed:false, consent_scope:"none", deletion_requested:false },
    { id:"contract", source_type:"contract", review_status:"approved", risk_level:"high", training_allowed:false, consent_scope:"none", deletion_requested:false, metadata:{} },
  ]));

  assert.ok(actions.some(item => item.scope === "reference" && item.action === "route_source_to_review" && item.sourceIds.includes("candidate")));
  assert.ok(actions.some(item => item.scope === "derivative" && item.action === "record_derivative_use_approval" && item.sourceIds.includes("contract")));
  assert.ok(actions.some(item => item.scope === "derivative" && item.action === "record_expert_reviewer_metadata" && item.sourceIds.includes("contract")));
  assert.ok(actions.some(item => item.scope === "training" && item.action === "collect_explicit_training_consent"));
  assert.ok(actions.every(item => item.blocksReadiness));
});
...
test("cold start ingestion batch manifest turns source plan into import batches", () => {
  const manifest = knowledgeBrainColdStartIngestionBatchManifest({
    knowledgeUnits:[
      { id:"ku-d07", domain:"D07", review_status:"approved" },
    ],
    evalCases:[],
  ...
  assert.deepEqual(manifest.map(item => item.id), [
    "ingestion-batch:phase_2_1_official_public",
    "ingestion-batch:phase_2_2_industry_templates",
    "ingestion-batch:phase_2_3_partner_cases",
    "ingestion-batch:phase_2_4_ai_assisted_long_tail",
  ]);
...
test("eval case false negative coverage gates high-risk tool release", () => {
  const blocked = evalCaseFalseNegativeCoverage([
    { id:"draft-fn", tool_id:"M4", review_status:"candidate", eval_type:"false_negative" },
    { id:"other-tool", tool_id:"M5", review_status:"approved", eval_type:"false_negative" },
  ], { toolId:"M4", minCases:1 });

  assert.equal(blocked.ready, false);
  assert.deepEqual(blocked.caseIds, []);
  assert.deepEqual(blocked.blockers, [{ gate:"false_negative_eval_cases", current:0, required:1 }]);
  assert.equal(blocked.actions[0].action, "create_false_negative_eval_cases");

  const ready = evalCaseFalseNegativeCoverage([
    { id:"approved-fn", tool_id:"M4", review_status:"approved", metadata:{ eval_type:"false_negative" } },
  ], { toolId:"M4" });

  assert.equal(ready.ready, true);
  assert.deepEqual(ready.caseIds, ["approved-fn"]);
});
...
test("tool validation run helpers version and filter release evidence", () => {
  const update = buildToolValidationRunUpdatePayload({
    id:"run-1",
    tool_id:"M4",
    mode:"internal_pilot",
    status:"pending",
    ...
    risk_level:"high",
    version:1,
    metadata:{ reviewer_role:"takken" },
  }, {
    status:"passed",
    review_status:"approved",
    metadata:{ approval_note:"internal pilot passed" },
  }, {
    changedBy:"takken-reviewer",
    reason:"internal_validation_passed",
    now:"2026-06-12T06:00:00.000Z",
  });

  assert.equal(update.record.version, 2);
  assert.equal(update.record.status, "passed");
  assert.equal(update.record.review_status, "approved");
  assert.equal(update.record.metadata.reviewed_by, "takken-reviewer");
  assert.equal(update.record.metadata.reviewed_at, "2026-06-12T06:00:00.000Z");
  assert.equal(update.quality.ok, true);
  assert.equal(update.validationReadiness.approvedInternalRuns, 1);
  const qualityActions = toolValidationRunQualityActions([
    { id:"bad-run", tool_id:"M4", mode:"external_release", status:"failed", review_status:"candidate", risk_level:"high", version:1, eval_case_ids:[], source_ids:[], evidence_ref_ids:[], false_negative_findings:2, metadata:{} },
  ]);
  assert.ok(qualityActions.some(item => item.action === "attach_eval_cases_to_validation_run" && item.runIds.includes("bad-run")));
  assert.ok(qualityActions.some(item => item.action === "move_validation_to_internal_pilot" && item.toolIds.includes("M4")));

  const filtered = filterToolValidationRunRecords([
    { ...update.record, updated_at:"2026-06-12T06:00:00.000Z" },
    { id:"run-2", tool_id:"M5", mode:"internal_pilot", status:"failed", review_status:"candidate", risk_level:"high", eval_case_ids:["eval-2"], source_ids:["src-2"], evidence_ref_ids:["ev-2"], false_negative_findings:1, metadata:{}, updated_at:"2026-06-12T07:00:00.000Z" },
  ], {
    toolIds:["M4"],
    statuses:["passed"],
    reviewStatuses:["approved"],
    sourceIds:["src-1"],
    evalCaseIds:["eval-1"],
    query:"internal pilot",
  });

  assert.deepEqual(filtered.map(item => item.id), ["run-1"]);
...
test("high-risk tools stay internal until cold start and eval set gates pass", () => {
  ...
  assert.equal(blocked.blockerSummary.byGate.false_negative_eval_set > 0, true);
  ...
  assert.equal(blocked.externalReleaseBlockedReasons.includes("false_negative_eval_cases"), true);
  ...
  assert.ok(blocked.blockers.some(item => item.gate === "false_negative_eval_set"));
  ...
  assert.ok(blocked.actions.some(item => item.readinessGate === "false_negative_eval_set" && item.action === "create_false_negative_eval_cases"));
...
test("high-risk tool readiness reports unsupported tool configuration action", () => {
  const readiness = knowledgeBrainHighRiskToolReadiness({
    ...
    toolId:"M9",
    requireFalseNegativeEvalCoverage:false,
...
test("high-risk tool readiness gates source contribution consent gaps", () => {
  const readiness = knowledgeBrainHighRiskToolReadiness({
    ...
    evalCases:[
      ...
      { id:"retrieval", source_id:"src-1", prompt:"Retrieve.", expected_behavior:"Cite evidence.", review_status:"approved", risk_level:"medium", version:1, evidence_ref_ids:["ev-1"] },
      { id:"boundary", source_id:"src-1", prompt:"Boundary.", expected_behavior:"Ask for expert review.", review_status:"approved", risk_level:"medium", version:1, metadata:{ eval_category:"boundary" } },
      { id:"false-negative", source_id:"src-1", tool_id:"M4", prompt:"Missed assessment risk.", expected_behavior:"Flag needs confirmation.", review_status:"approved", risk_level:"high", version:1, metadata:{ eval_type:"false_negative", eval_category:"boundary" } },
    ],
  }, {
    toolId:"M4",
    coldStartOptions:{
...
test("source deletion impact summary previews downstream archive and relink work", () => {
  const impact = sourceDeletionImpactSummary({
    sourceId:"src-free-tier",
    sources:[
      { id:"src-free-tier", review_status:"approved", risk_level:"low", training_allowed:true, deletion_requested:false },
    ],
    knowledgeUnits:[
      { id:"ku-1", source_id:"src-free-tier", review_status:"approved", risk_level:"medium", evidence_ref_ids:["ev-1"] },
      { id:"ku-other", source_id:"src-other", review_status:"approved", risk_level:"low", evidence_ref_ids:["ev-1"] },
    ],
    evidenceRefs:[
      { id:"ev-1", source_id:"src-free-tier", review_status:"approved", risk_level:"low" },
    ],
    japaneseRealEstateRecords:[
      { id:"risk-1", entity_type:"risk", source_id:"src-free-tier", review_status:"candidate", risk_level:"high", evidence_ref_ids:["ev-1"] },
    ],
    calculationRuns:[
...
test("knowledge brain inventory stats expose review, risk, evidence, and training counts", () => {
  const stats = knowledgeBrainInventoryStats({
    ...
    evalCases:[{ id:"eval-1", source_id:"src-1", prompt:"Cite evidence.", expected_behavior:"Cite approved evidence.", review_status:"approved", risk_level:"medium", evidence_ref_ids:[], version:1, metadata:{ eval_category:"retrieval" } }],
    calculationRuns:[
    ...
    toolValidationRuns:[
      { id:"run-1", tool_id:"M4", mode:"internal_pilot", status:"passed", review_status:"approved", risk_level:"high", version:1, eval_case_ids:["eval-1"], source_ids:["src-1"], evidence_ref_ids:["ev-1"], false_negative_findings:0, metadata:{ reviewed_by:"takken", reviewed_at:"2026-06-12T00:00:00.000Z" } },
      { id:"run-2", tool_id:"M4", mode:"internal_pilot", status:"passed", review_status:"approved", risk_level:"high", version:1, eval_case_ids:["eval-1"], source_ids:["src-1"], evidence_ref_ids:["ev-1"], false_negative_findings:0, metadata:{ reviewed_by:"takken", reviewed_at:"2026-06-12T01:00:00.000Z" } },
  ...
  assert.equal(stats.sourceReferenceSearchEligibilityReport[0].eligible, true);
  assert.deepEqual(stats.sourceReferenceSearchEligibilityReport[2].reasons, ["deletion_requested", "source_archived", "source_not_approved", "missing_approved_review_metadata"]);
  assert.ok(stats.sourceReferenceSearchEligibilityActions.some(item => item.action === "record_expert_reviewer_metadata"));
  assert.ok(stats.sourceReferenceSearchEligibilityActions.some(item => item.action === "honor_source_deletion_or_restore_with_owner_approval"));
  ...
  assert.equal(stats.toolValidationRuns, 4);
  assert.equal(stats.toolValidationRunReviewStatus.approved, 3);
  assert.equal(stats.toolValidationRunReviewStatus.candidate, 1);
  assert.equal(stats.toolValidationRunRiskLevels.high, 4);
  assert.deepEqual(stats.toolValidationRunStatuses, { passed:3, failed:1 });
  assert.equal(stats.invalidToolValidationRuns, 1);
  assert.equal(stats.toolValidationRunQualityIssues.missing_eval_case_ids, 1);
  ...
  assert.equal(stats.knowledgeBrainColdStartIngestionBatchManifest[2].requiresExplicitUseAgreement, true);
  ...
  assert.equal(stats.evalCaseReviewStatus.approved, 1);
...
test("knowledge brain review queue action summary groups reviewer work", () => {
  const summary = knowledgeBrainReviewQueueActionSummary([
  ...
  assert.ok(summary.some(item => item.action === "assign_expert_reviewer" && item.current === 1));
