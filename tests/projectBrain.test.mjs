...
test("tool validation run helpers version and filter release evidence", () => {
  ...
  assert.deepEqual(filterToolValidationRunQualityActions(qualityActions, {
    issues:["non_internal_validation_mode"],
    actions:["move_validation_to_internal_pilot"],
    toolIds:["M4"],
    runIds:["bad-run"],
    blocksRelease:true,
    query:"bad-run",
  }).map(item => item.action), ["move_validation_to_internal_pilot"]);
