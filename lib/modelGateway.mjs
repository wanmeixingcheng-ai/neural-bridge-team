...
function localOnlyBlockMessage(modelKey, apiKeys = {}, lang = "zh") {
  if (!outboundBlockedByLocalOnly(modelKey, apiKeys)) return "";
  const provider = outboundProviderLabel(modelKey, apiKeys) || "Codex/local task gateway";
  if (lang === "ja") return `Local-only モードのため送信を停止しました。対象：${provider}`;
  if (lang === "en") return `Local-only mode blocked this send. Target: ${provider}`;
  return `Local-only 模式已阻止本次发送。目标：${provider}`;
}
