...
function knowledgeBrainExportManifest(payload = {}) {
  ...
  const manifest = brainStores.reduce((manifest, store) => {
    ...
    const storeSummary = items.reduce((summary, item = {}) => {
      ...
      const usesSourceIds = ["calculation_runs", "property_dossiers", "tool_validation_runs"].includes(store.name);
      ...
      if (store.name !== "source_registry" && (!Array.isArray(item.evidence_ref_ids) || item.evidence_ref_ids.length === 0) && ["high", "restricted"].includes(riskLevel)) {
