...
function normalizeImportedSourceRegistryRecord(source = {}) {
  ...
  if (sourceType === "reins_user_upload" && !reinsManualMethods.has(record.collection_method)) {
    ...
    record.metadata = {
      ...
      import_warnings:[...new Set([...warnings, "reins_collection_method_sanitized"])],
    };
  }
  const trainingBlocked =
    !["opt_in", "explicit_opt_in"].includes(consentScope) ||
    record.deletion_requested === true ||
    ["high", "restricted"].includes(riskLevel) ||
    HIGH_RISK_SOURCE_TYPES.includes(sourceType);
  if (trainingBlocked) record.training_allowed = false;
  return record;
}
...
function trainingEligibleSources(sources = []) {
  return sources.filter(source =>
    ...
    ["opt_in", "explicit_opt_in"].includes(source.consent_scope) &&
...
async function importKnowledgeLibrary(payload) {
  ...
  await new Promise((resolve, reject) => {
    ...
    for (const store of brainStores) {
      brainPayload[store.name].forEach(item => tx.objectStore(store.name).put(
        store.name === KNOWLEDGE_BRAIN_STORES.sourceRegistry.name
