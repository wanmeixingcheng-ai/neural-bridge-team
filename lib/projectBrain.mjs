...
function knowledgeBrainReviewQueueItems({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], limit = 100 } = {}) {
  const items = [];
...
async function listKnowledgeGovernanceRecords(recordType, { sourceId = "", reviewStatuses = [], includeArchived = false } = {}) {
  const normalized = `${recordType || ""}`.trim();
  const definition = GOVERNANCE_RECORD_BUILDERS[normalized];
  if (!definition) {
    throw new Error("recordType must be one of: policy_rule, scenario, eval_case");
  }
  const statusSet = new Set(reviewStatuses);
