...
function sourceReferenceSearchEligibilityReport(sources = []) {
  return sources.map(source => {
    const permissions = sourceUsagePermissions(source);
    const reasons = [
      ...permissions.reference.reasons,
      ...(approvedRecordHasReviewMetadata(source) ? [] : ["missing_approved_review_metadata"]),
    ];
    return {
      source_id:source.id || "",
      title:source.title || "",
...
function filterSourceReferenceSearchEligibilityReport(items = [], { sourceTypes = [], reviewStatuses = [], riskLevels = [], eligible, reasons = [], query = "" } = {}) {
...
function sourceReferenceSearchEligibilityActions(report = []) {
  const actionByReason = {
    deletion_requested:"honor_source_deletion_or_restore_with_owner_approval",
    missing_approved_review_metadata:"record_expert_reviewer_metadata",
    source_archived:"restore_or_replace_source",
    source_not_approved:"route_source_to_review",
  };
  const actionsByReason = new Map();
  for (const item of report) {
    for (const reason of item.reasons || []) {
      const action = actionByReason[reason] || "manual_reference_search_review";
      const current = actionsByReason.get(reason) || {
        id:`source-reference-search-action:${reason}`,
        reason,
        action,
        sourceIds:[],
        current:0,
      ...
      if (item.source_id && !current.sourceIds.includes(item.source_id)) {
        current.sourceIds.push(item.source_id);
      }
      current.current = current.sourceIds.length;
      actionsByReason.set(reason, current);
    }
  }
  return [...actionsByReason.values()].sort((a, b) => a.reason.localeCompare(b.reason));
}

function approvedKnowledgeUnitSearchResults({ query, sources = [], units = [] } = {}) {
  const keyword = `${query || ""}`.trim().toLowerCase();
  if (!keyword) return [];
  const approvedSourceIds = new Set(sourceReferenceSearchEligibilityReport(sources)
...
function approvedKnowledgeBrainSearchResults({ query, sources = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [] } = {}) {
  const keyword = `${query || ""}`.trim().toLowerCase();
  if (!keyword) return [];
  const approvedSourceIds = new Set(sourceReferenceSearchEligibilityReport(sources)
    .filter(source => source.eligible)
  ...
  const approvedCalculationRunIds = new Set(calculationRuns
    .filter(record => approvedRecordHasReviewMetadata(record))
    .filter(record => Array.isArray(record.source_ids) && record.source_ids.length > 0 && record.source_ids.every(sourceId => approvedSourceIds.has(sourceId)))
    .map(record => record.id)
    .filter(Boolean));
  const dossierHits = propertyDossiers
    .filter(record => approvedRecordHasReviewMetadata(record) && validatePropertyDossierRecord(record).ok)
    .filter(record => Array.isArray(record.source_ids) && record.source_ids.length > 0 && record.source_ids.every(sourceId => approvedSourceIds.has(sourceId)))
    .filter(record => !Array.isArray(record.calculation_run_ids) || record.calculation_run_ids.every(runId => approvedCalculationRunIds.has(runId)))
    .map(record => buildHit({
      type:"property_dossier",
      id:record.id,
      sourceIds:record.source_ids || [],
      title:record.title,
      content:JSON.stringify({
        ...
        jre_record_ids:record.jre_record_ids,
        calculation_run_ids:record.calculation_run_ids,
        summary_snapshot:record.summary_snapshot,
  ...
  return [...evidenceHits, ...governanceHits, ...jreHits, ...calculationHits, ...dossierHits]
    .filter(Boolean)
    .sort((a, b) => b.score - a.score);
}
...
function knowledgeBrainColdStartDomainPlan(input = {}) {
  ...
  const groups = Object.fromEntries(Object.entries(KNOWLEDGE_BRAIN_COLD_START_DOMAIN_GROUPS).map(([phase, group]) => {
    ...
    return [phase, {
      ...output,
      ...coldStartExecutionMetrics(output),
    }];
  }));
  return {
    groups,
...
function coldStartExecutionMetrics(group = {}) {
  const minTarget = Number(group.approvedKnowledgeUnitTarget?.min || 0);
  const domains = Array.isArray(group.domains) ? group.domains : [];
  const approved = Number(group.approvedKnowledgeUnits || 0);
  const approvedKnowledgeUnitDeficit = Math.max(0, minTarget - approved);
  const perDomainTarget = minTarget > 0 ? Math.ceil(minTarget / Math.max(1, domains.length)) : 0;
  return {
    approvedKnowledgeUnitDeficit,
    targetProgressPercent:minTarget > 0 ? Math.min(100, Math.round((approved / minTarget) * 100)) : 100,
    suggestedNextApprovedUnits:approvedKnowledgeUnitDeficit === 0
      ? 0
      : Math.min(approvedKnowledgeUnitDeficit, Math.max((group.missingApprovedKnowledgeUnitDomains || []).length, perDomainTarget)),
    evalCaseDomainDeficit:(group.missingEvalCaseDomains || []).length,
  };
}
  ...
  return Object.entries(plan.groups)
    .map(([phase, group], index) => {
      ...
      return {
        ...
        approvedKnowledgeUnitDeficit:group.approvedKnowledgeUnitDeficit,
        targetProgressPercent:group.targetProgressPercent,
        suggestedNextApprovedUnits:group.suggestedNextApprovedUnits,
        evalCaseDomainDeficit:group.evalCaseDomainDeficit,
...
function knowledgeBrainColdStartIngestionBatchManifest(input = {}) {
  return knowledgeBrainColdStartSourceAcquisitionPlan(input).map(item => ({
    ...
    targetProgressPercent:item.targetProgressPercent,
    suggestedNextApprovedUnits:item.suggestedNextApprovedUnits,
    evalCaseDomainDeficit:item.evalCaseDomainDeficit,
...
function sourceDeletionImpactSummary({ sourceId = "", sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], toolValidationRuns = [] } = {}) {
  const safeSourceId = `${sourceId || ""}`.trim();
  const source = sources.find(item => item.id === safeSourceId) || null;
  const linked = {
    knowledgeUnits:knowledgeUnits.filter(item => item.source_id === safeSourceId),
    policyRules:policyRules.filter(item => item.source_id === safeSourceId),
    scenarios:scenarios.filter(item => item.source_id === safeSourceId),
    evalCases:evalCases.filter(item => item.source_id === safeSourceId),
    evidenceRefs:evidenceRefs.filter(item => item.source_id === safeSourceId),
    japaneseRealEstateRecords:japaneseRealEstateRecords.filter(item => item.source_id === safeSourceId),
    calculationRuns:calculationRuns.filter(item => Array.isArray(item.source_ids) && item.source_ids.includes(safeSourceId)),
    propertyDossiers:propertyDossiers.filter(item => Array.isArray(item.source_ids) && item.source_ids.includes(safeSourceId)),
    toolValidationRuns:toolValidationRuns.filter(item => Array.isArray(item.source_ids) && item.source_ids.includes(safeSourceId)),
  };
  const linkedRecords = Object.values(linked).flat();
  const byStore = Object.fromEntries(Object.entries(linked).map(([key, items]) => [key, items.length]));
  const linkedRecordIds = Object.fromEntries(Object.entries(linked).map(([key, items]) => [key, items.map(item => item.id).filter(Boolean).slice(0, 50)]));
  const approvedLinkedRecords = linkedRecords.filter(item => item.review_status === "approved");
  const highRiskLinkedRecords = linkedRecords.filter(item => ["high", "restricted"].includes(item.risk_level));
  const evidenceIds = new Set(linked.evidenceRefs.map(item => item.id).filter(Boolean));
  const recordsUsingLinkedEvidence = [
    ...knowledgeUnits.map(item => ({ ...item, target_type:"knowledge_unit" })),
    ...policyRules.map(item => ({ ...item, target_type:"policy_rule" })),
    ...scenarios.map(item => ({ ...item, target_type:"scenario" })),
    ...evalCases.map(item => ({ ...item, target_type:"eval_case" })),
    ...japaneseRealEstateRecords.map(item => ({ ...item, target_type:`jre_${item.entity_type || "record"}` })),
    ...calculationRuns.map(item => ({ ...item, target_type:"calculation_run" })),
    ...propertyDossiers.map(item => ({ ...item, target_type:"property_dossier" })),
    ...toolValidationRuns.map(item => ({ ...item, target_type:"tool_validation_run" })),
  ].filter(item => Array.isArray(item.evidence_ref_ids) && item.evidence_ref_ids.some(id => evidenceIds.has(id)));
  const actions = [];
  if (!source) {
    actions.push({
      action:"verify_source_id_before_deletion",
      current:0,
      required:1,
      blocksDeletion:true,
    });
  }
  if (source?.training_allowed === true) {
    actions.push({
      action:"disable_training_and_record_withdrawal_audit",
      current:1,
      required:0,
      blocksDeletion:false,
    });
  }
  if (approvedLinkedRecords.length > 0) {
    actions.push({
      action:"archive_or_relink_approved_source_backed_records",
      current:approvedLinkedRecords.length,
      required:0,
      blocksDeletion:false,
    });
  }
  if (highRiskLinkedRecords.length > 0) {
    actions.push({
      action:"route_high_risk_deletion_impact_to_expert_review",
      current:highRiskLinkedRecords.length,
      required:0,
      blocksDeletion:false,
    });
  }
  if (recordsUsingLinkedEvidence.length > 0) {
    actions.push({
      action:"relink_or_downgrade_records_using_deleted_evidence",
      current:recordsUsingLinkedEvidence.length,
      required:0,
      blocksDeletion:false,
    });
  }
  return {
    ...
    sourceFound:!!source,
    sourceReviewStatus:source?.review_status || "missing",
    sourceRiskLevel:source?.risk_level || "unknown",
    currentTrainingAllowed:source?.training_allowed === true,
    wouldDisableTraining:source?.training_allowed === true,
    linkedRecords:linkedRecords.length,
    byStore,
    linkedRecordIds,
    approvedLinkedRecords:approvedLinkedRecords.length,
    highRiskLinkedRecords:highRiskLinkedRecords.length,
    recordsUsingLinkedEvidence:recordsUsingLinkedEvidence.length,
    recordsUsingLinkedEvidenceIds:recordsUsingLinkedEvidence.map(item => `${item.target_type}:${item.id || ""}`).filter(Boolean).slice(0, 50),
    actions,
  };
}
...
function knowledgeBrainExportManifest(payload = {}) {
  const brainStores = Object.values(KNOWLEDGE_BRAIN_STORES);
  const manifest = brainStores.reduce((manifest, store) => {
    ...
    const storeSummary = items.reduce((summary, item = {}) => {
      ...
      const usesSourceIds = ["calculation_runs", "property_dossiers", "tool_validation_runs", "runtime_gate_events"].includes(store.name);
      const usesEvidenceRefs = store.name !== "runtime_gate_events";
      if (store.name !== "source_registry" && !usesSourceIds && !item.source_id) summary.missingSourceId += 1;
      if (usesSourceIds && (!Array.isArray(item.source_ids) || item.source_ids.length === 0)) summary.missingSourceId += 1;
      if (store.name !== "source_registry" && usesEvidenceRefs && (!Array.isArray(item.evidence_ref_ids) || item.evidence_ref_ids.length === 0) && ["high", "restricted"].includes(riskLevel)) {
  ...
  const stats = knowledgeBrainInventoryStats(knowledgeBrainPayloadRecords(payload));
  return {
    ...manifest,
    reviewQueue:stats.reviewQueue,
    reviewQueueActionSummary:stats.reviewQueueActionSummary,
    referenceIntegrityIssues:stats.referenceIntegrityIssues,
    referenceIntegrityActions:stats.knowledgeBrainReferenceIntegrityActions.slice(0, 10),
    sourceTrainingEligibilityBlockedReasons:stats.sourceTrainingEligibilityBlockedReasons,
    sourceUsagePermissionBlockedReasons:stats.sourceUsagePermissionBlockedReasons,
    sourceReferenceSearchEligibilityActions:stats.sourceReferenceSearchEligibilityActions,
    sourceContributionConsentActions:stats.sourceContributionConsentActions,
    highRiskToolValidationReadiness:stats.highRiskToolValidationReadiness,
    highRiskToolFalseNegativeCoverage:stats.highRiskToolFalseNegativeCoverage,
    invalidToolValidationRuns:stats.invalidToolValidationRuns,
    toolValidationRunQualityIssues:stats.toolValidationRunQualityIssues,
    toolValidationRunQualityActions:stats.toolValidationRunQualityActions,
    invalidRuntimeGateEvents:stats.invalidRuntimeGateEvents,
    runtimeGateEventQualityIssues:stats.runtimeGateEventQualityIssues,
  };
}

function knowledgeBrainPayloadRecords(payload = {}) {
  const storeRecords = name => Array.isArray(payload?.[name]) ? payload[name] : [];
  return {
    sources:storeRecords(KNOWLEDGE_BRAIN_STORES.sourceRegistry.name),
    knowledgeUnits:storeRecords(KNOWLEDGE_BRAIN_STORES.knowledgeUnits.name),
    evidenceRefs:storeRecords(KNOWLEDGE_BRAIN_STORES.evidenceRefs.name),
    policyRules:storeRecords(KNOWLEDGE_BRAIN_STORES.policyRules.name),
    scenarios:storeRecords(KNOWLEDGE_BRAIN_STORES.scenarios.name),
    evalCases:storeRecords(KNOWLEDGE_BRAIN_STORES.evalCases.name),
    calculationRuns:storeRecords(KNOWLEDGE_BRAIN_STORES.calculationRuns.name),
    propertyDossiers:storeRecords(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name),
    japaneseRealEstateRecords:JRE_ENTITY_TYPES.flatMap(type => storeRecords(japaneseRealEstateStoreName(type))),
    toolValidationRuns:storeRecords(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name),
    runtimeGateEvents:storeRecords(KNOWLEDGE_BRAIN_STORES.runtimeGateEvents.name),
  };
}
...
function knowledgeBrainImportAuditActions(summary = {}) {
  ...
  if ((summary.governance?.invalidRuntimeGateEvents || 0) > 0) {
    actions.push({
      id:"knowledge-import-action:invalid_runtime_gate_events",
      action:"review_runtime_gate_event_quality",
      current:summary.governance.invalidRuntimeGateEvents,
      required:0,
      blocksImport:false,
    });
  }
...
function knowledgeBrainImportAuditSummary(payload = {}) {
  ...
  const normalizedBrainPayload = Object.fromEntries(brainStores.map(store => [
    store.name,
    (Array.isArray(payload?.[store.name]) ? payload[store.name] : []).map(item => normalizeImportedKnowledgeBrainRecord(store.name, item)),
  ]));
  const summary = brainStores.reduce((summary, store) => {
    const items = Array.isArray(payload?.[store.name]) ? payload[store.name] : [];
    const normalizedItems = normalizedBrainPayload[store.name] || [];
    const storeSummary = {
      total:items.length,
      trainingDisabled:0,
      reviewDowngraded:0,
  ...
  const stats = knowledgeBrainInventoryStats(knowledgeBrainPayloadRecords({
    ...normalizedBrainPayload,
    [KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name]:Array.isArray(payload?.tool_validation_runs)
      ? payload.tool_validation_runs
      : normalizedBrainPayload[KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name],
  }));
  summary.governance = {
    reviewQueue:stats.reviewQueue,
    reviewQueueActionSummary:stats.reviewQueueActionSummary,
...
function evalCaseFalseNegativeCoverage(evalCases = [], {
  toolId = "",
  minCases = 1,
  requireApproved = true,
} = {}) {
  const normalizedToolId = `${toolId || ""}`.trim();
  const cases = evalCases.filter(evalCase => {
    const metadata = evalCase.metadata && typeof evalCase.metadata === "object" ? evalCase.metadata : {};
    const evalType = `${evalCase.eval_type || metadata.eval_type || metadata.eval_kind || ""}`.trim();
    const caseToolId = `${evalCase.tool_id || metadata.tool_id || ""}`.trim();
    const matchesTool = !normalizedToolId || !caseToolId || caseToolId === normalizedToolId;
    const approved = !requireApproved || evalCase.review_status === "approved";
    return matchesTool && approved && evalType === "false_negative";
  });
  const blockers = cases.length >= minCases ? [] : [{
    gate:"false_negative_eval_cases",
  ...
  return {
    ...
    total:cases.length,
    caseIds:cases.map(evalCase => evalCase.id).filter(Boolean).slice(0, 50),
    gates:{ minCases, requireApproved },
    blockers,
    actions:blockers.map(blocker => ({
      id:`eval-false-negative-action:${blocker.gate}`,
      gate:blocker.gate,
      action:"create_false_negative_eval_cases",
      current:blocker.current,
      required:blocker.required,
      blocksReadiness:true,
    })),
...
function countPropertyDossierIssues(records = []) {
  return records.reduce((counts, record) => {
    const quality = validatePropertyDossierRecord(record);
    if (quality.ok) return counts;
    counts.invalid += 1;
    for (const issue of quality.issues) {
      counts.byIssue[issue] = (counts.byIssue[issue] || 0) + 1;
...
function countToolValidationRunIssues(records = []) {
  return records.reduce((counts, record) => {
    const quality = validateToolValidationRunRecord(record);
    if (quality.ok) return counts;
    counts.invalid += 1;
    for (const issue of quality.issues) {
      counts.byIssue[issue] = (counts.byIssue[issue] || 0) + 1;
...
function countRuntimeGateEventIssues(records = []) {
  return records.reduce((counts, record) => {
    const quality = validateRuntimeGateEventRecord(record);
    if (quality.ok) return counts;
    counts.invalid += 1;
    for (const issue of quality.issues) {
      counts.byIssue[issue] = (counts.byIssue[issue] || 0) + 1;
...
function toolValidationRunQualityActions(records = []) {
  const actionByIssue = {
    missing_tool_id:"assign_high_risk_tool_id",
    missing_mode:"set_internal_pilot_mode",
    missing_status:"record_validation_run_status",
    missing_review_status:"route_validation_run_to_review",
    missing_risk_level:"classify_validation_run_risk",
    missing_version:"assign_validation_run_version",
    missing_eval_case_ids:"attach_eval_cases_to_validation_run",
    missing_source_ids:"attach_sources_to_validation_run",
    missing_evidence_ref_ids:"attach_evidence_to_validation_run",
    false_negative_findings_present:"repair_false_negative_eval_failures",
    non_internal_validation_mode:"move_validation_to_internal_pilot",
    approved_validation_missing_reviewer_metadata:"record_expert_validation_signoff",
  };
  ...
  for (const record of records) {
    ...
    for (const issue of quality.issues) {
      const action = actionByIssue[issue] || "manual_tool_validation_review";
      const current = actionsByIssue.get(issue) || {
        id:`tool-validation-quality-action:${issue}`,
        issue,
        action,
        runIds:[],
        toolIds:[],
      ...
      if (record.id && !current.runIds.includes(record.id)) current.runIds.push(record.id);
      if (record.tool_id && !current.toolIds.includes(record.tool_id)) current.toolIds.push(record.tool_id);
      current.current = current.runIds.length;
      actionsByIssue.set(issue, current);
    }
  }
  return [...actionsByIssue.values()].sort((a, b) => a.issue.localeCompare(b.issue));
}

function filterToolValidationRunQualityActions(actions = [], { issues = [], actions:actionNames = [], toolIds = [], runIds = [], blocksRelease, query = "" } = {}) {
  ...
  return actions
    ...
    .filter(item => !toolIdSet.size || (Array.isArray(item.toolIds) && item.toolIds.some(toolId => toolIdSet.has(toolId))))
    .filter(item => !runIdSet.size || (Array.isArray(item.runIds) && item.runIds.some(runId => runIdSet.has(runId))))
    .filter(item => blocksRelease === undefined || item.blocksRelease === blocksRelease)
    .filter(item => {
      if (!keyword) return true;
      return [
        item.issue,
        ...
        ...(Array.isArray(item.runIds) ? item.runIds : []),
      ].join("\n").toLowerCase().includes(keyword);
    });
}
...
function validateKnowledgeBrainReferenceIntegrity({ sources = [], evidenceRefs = [], knowledgeUnits = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], toolValidationRuns = [] } = {}) {
  ...
  const evalCaseById = new Map(evalCases.filter(evalCase => evalCase?.id).map(evalCase => [evalCase.id, evalCase]));
  const calculationRunById = new Map(calculationRuns.filter(run => run?.id).map(run => [run.id, run]));
  const jreRecordById = new Map(japaneseRealEstateRecords.filter(record => record?.id).map(record => [record.id, record]));
  const issues = [];
  ...
  const checkEvalCases = (target, evalCaseIds = []) => {
    for (const evalCaseId of evalCaseIds) {
      const evalCase = evalCaseById.get(evalCaseId);
      if (!evalCase) {
        addIssue(target, "missing_eval_case_ref", { eval_case_id:evalCaseId });
        continue;
      }
      if (target.review_status === "approved" && evalCase.review_status !== "approved") {
        addIssue(target, "approved_validation_unapproved_eval_case", { eval_case_id:evalCaseId, eval_case_review_status:evalCase.review_status || "unknown" });
      }
    }
  };
  const checkCalculationRuns = (target, calculationRunIds = []) => {
    for (const calculationRunId of calculationRunIds) {
      const calculationRun = calculationRunById.get(calculationRunId);
      if (!calculationRun) {
        addIssue(target, "missing_calculation_run_ref", { calculation_run_id:calculationRunId });
        continue;
      }
      if (target.review_status === "approved" && calculationRun.review_status !== "approved") {
        addIssue(target, "approved_dossier_unapproved_calculation_run", { calculation_run_id:calculationRunId, calculation_run_review_status:calculationRun.review_status || "unknown" });
      }
    }
  };
  const checkJreRecords = (target, jreRecordIdsByType = {}) => {
    const jreRecordIds = Object.values(jreRecordIdsByType && typeof jreRecordIdsByType === "object" && !Array.isArray(jreRecordIdsByType) ? jreRecordIdsByType : {})
      .flatMap(ids => Array.isArray(ids) ? ids : []);
    for (const jreRecordId of jreRecordIds) {
      const jreRecord = jreRecordById.get(jreRecordId);
      if (!jreRecord) {
        addIssue(target, "missing_jre_record_ref", { jre_record_id:jreRecordId });
        continue;
      }
      if (target.review_status === "approved" && jreRecord.review_status !== "approved") {
        addIssue(target, "approved_dossier_unapproved_jre_record", { jre_record_id:jreRecordId, jre_record_review_status:jreRecord.review_status || "unknown" });
      }
    }
  };
  for (const unit of knowledgeUnits) {
  ...
  for (const dossier of propertyDossiers) {
    const target = { target_type:"property_dossier", target_id:dossier.id || "", review_status:dossier.review_status, risk_level:dossier.risk_level, metadata:dossier.metadata };
    checkReviewMetadata(target);
    for (const sourceId of Array.isArray(dossier.source_ids) ? dossier.source_ids : []) checkSource(target, sourceId);
    checkEvidence(target, Array.isArray(dossier.evidence_ref_ids) ? dossier.evidence_ref_ids : []);
    checkCalculationRuns(target, Array.isArray(dossier.calculation_run_ids) ? dossier.calculation_run_ids : []);
    checkJreRecords(target, dossier.jre_record_ids);
  }
  for (const run of toolValidationRuns) {
    const target = { target_type:"tool_validation_run", target_id:run.id || "", review_status:run.review_status, risk_level:run.risk_level, metadata:run.metadata };
    checkReviewMetadata(target);
    for (const sourceId of Array.isArray(run.source_ids) ? run.source_ids : []) checkSource(target, sourceId);
    checkEvidence(target, Array.isArray(run.evidence_ref_ids) ? run.evidence_ref_ids : []);
    checkEvalCases(target, Array.isArray(run.eval_case_ids) ? run.eval_case_ids : []);
  }
  return {
    ok:issues.length === 0,
    issues,
...
function knowledgeBrainReferenceIntegrityActions(referenceIntegrity = {}, filters = {}) {
  ...
  const actionByIssue = {
    ...
    missing_eval_case_ref:"attach_eval_case_or_archive_validation_run",
    approved_validation_unapproved_eval_case:"approve_eval_case_before_validation_run",
    missing_calculation_run_ref:"attach_calculation_run_or_archive_dossier",
    approved_dossier_unapproved_calculation_run:"approve_calculation_run_before_dossier",
    missing_jre_record_ref:"attach_jre_record_or_archive_dossier",
    approved_dossier_unapproved_jre_record:"approve_jre_record_before_dossier",
    missing_reviewed_by:"record_expert_reviewer_metadata",
    missing_reviewed_at:"record_expert_reviewer_metadata",
  };
  const actions = issues.map(issue => ({
    ...
    blocks_approval:[
      ...
      "approved_dossier_unapproved_calculation_run",
...
function knowledgeBrainReviewerRoleSummary({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [] } = {}) {
  const records = [
    ...
    ...propertyDossiers.map(record => ({ ...record, target_type:"property_dossier" })),
  ];
...
function knowledgeBrainReviewQueueItems({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], runtimeGateEvents = [], limit = 100, targetTypes = [], sourceIds = [], reviewStatuses = [], riskLevels = [], reasons = [], query = "", includeReferenceIntegrity = true } = {}) {
  const items = [];
  ...
  for (const record of propertyDossiers) {
    rememberTarget("property_dossier", record.id || "", record);
    addItem({
      target_type:"property_dossier",
      target_id:record.id || "",
      source_ids:record.source_ids || [],
      review_status:record.review_status,
      risk_level:record.risk_level,
      reasons:reviewReasons(record),
      quality:validatePropertyDossierRecord(record),
      metadata:queueMetadataFor(record),
    });
  }
  for (const record of runtimeGateEvents) {
    addItem({
      target_type:"runtime_gate_event",
      target_id:record.id || "",
      source_ids:record.source_ids || [],
      review_status:record.review_status,
      risk_level:record.risk_level,
      reasons:reviewReasons(record),
      quality:validateRuntimeGateEventRecord(record),
      metadata:queueMetadataFor({
        ...record,
        title:record.action || record.tool_id || "",
        provider:record.route_model || "",
      }),
    });
  }
...
function knowledgeBrainReviewQueueActionSummary(items = []) {
  const actionByReason = {
    ...
    missing_tool_id:"assign_runtime_gate_tool_id",
    missing_action:"record_runtime_gate_action",
    missing_route_model:"record_runtime_gate_route_model",
    invalid_policy_result:"repair_runtime_gate_event_payload",
    invalid_output_quality:"repair_runtime_gate_event_payload",
    invalid_source_ids:"repair_runtime_gate_event_references",
    invalid_knowledge_ids:"repair_runtime_gate_event_references",
    external_allowed_with_blocked_reason:"fix_runtime_gate_route_conflict",
    high_risk_external_model_allowed:"block_high_risk_external_model_route",
    invalid_response_status:"repair_runtime_gate_response_status",
  };
...
function knowledgeBrainReviewQueueSummary({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], runtimeGateEvents = [] } = {}) {
  const highRiskNeedsExpert = (item = {}) => ["high", "restricted"].includes(item.risk_level) && item.review_status !== "approved";
  ...
  const propertyDossierQuality = propertyDossiers.map(record => ({ record, quality:validatePropertyDossierRecord(record) }));
  const runtimeGateEventQuality = runtimeGateEvents.map(record => ({ record, quality:validateRuntimeGateEventRecord(record) }));
  return {
    total:
      ...
      propertyDossiers.filter(needsKnowledgeBrainReview).length +
      runtimeGateEvents.filter(needsKnowledgeBrainReview).length,
    ...
    propertyDossiers:propertyDossiers.filter(needsKnowledgeBrainReview).length,
    runtimeGateEvents:runtimeGateEvents.filter(needsKnowledgeBrainReview).length,
    highRiskExpertReview:
      ...
      propertyDossiers.filter(highRiskNeedsExpert).length +
      runtimeGateEvents.filter(highRiskNeedsExpert).length,
    ...
    invalidPropertyDossiers:propertyDossierQuality.filter(item => !item.quality.ok).length,
    invalidPropertyDossierIds:propertyDossierQuality.filter(item => !item.quality.ok).map(item => item.record.id).filter(Boolean).slice(0, 50),
    invalidRuntimeGateEvents:runtimeGateEventQuality.filter(item => !item.quality.ok).length,
    invalidRuntimeGateEventIds:runtimeGateEventQuality.filter(item => !item.quality.ok).map(item => item.record.id).filter(Boolean).slice(0, 50),
  };
}

function knowledgeBrainInventoryStats({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], toolValidationRuns = [], runtimeGateEvents = [] } = {}) {
  ...
  const propertyDossierIssues = countPropertyDossierIssues(propertyDossiers);
  const toolValidationIssues = countToolValidationRunIssues(toolValidationRuns);
  const runtimeGateEventIssues = countRuntimeGateEventIssues(runtimeGateEvents);
  const reviewQueue = knowledgeBrainReviewQueueSummary({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns, propertyDossiers, runtimeGateEvents });
  const reviewQueueItems = knowledgeBrainReviewQueueItems({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns, propertyDossiers, runtimeGateEvents, limit:50 });
  ...
  const reviewerRoleSummary = knowledgeBrainReviewerRoleSummary({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns, propertyDossiers });
  ...
  const referenceSearchEligibilityReport = sourceReferenceSearchEligibilityReport(sources);
  const trainingEligibilityReport = sourceTrainingEligibilityReport(sources);
  const highRiskToolValidation = Object.fromEntries(["M4", "M5"].map(toolId => [
    toolId,
    highRiskToolValidationReadiness(toolValidationRuns, { toolId }),
  ]));
  const highRiskToolFalseNegativeCoverage = Object.fromEntries(["M4", "M5"].map(toolId => [
    toolId,
    evalCaseFalseNegativeCoverage(evalCases, { toolId }),
  ]));
  const sourceDeletionImpactSummaries = sources
    .filter(source => source.deletion_requested === true)
    .map(source => sourceDeletionImpactSummary({
      sourceId:source.id,
      sources,
      knowledgeUnits,
      evidenceRefs,
  ...
  return {
    ...
    sourceReferenceSearchEligibilityReport:referenceSearchEligibilityReport.slice(0, 50),
    sourceReferenceSearchEligibilityActions:sourceReferenceSearchEligibilityActions(referenceSearchEligibilityReport).slice(0, 50),
    sourceDeletionImpactSummaries:sourceDeletionImpactSummaries.slice(0, 20),
    highRiskToolValidationReadiness:highRiskToolValidation,
    highRiskToolFalseNegativeCoverage,
    ...
    propertyDossiers:propertyDossiers.length,
    propertyDossierReviewStatus:countByStatus(propertyDossiers),
    propertyDossierRiskLevels:countByRiskLevel(propertyDossiers),
    invalidPropertyDossiers:propertyDossierIssues.invalid,
    propertyDossierQualityIssues:propertyDossierIssues.byIssue,
    toolValidationRuns:toolValidationRuns.length,
    toolValidationRunReviewStatus:countByStatus(toolValidationRuns),
    toolValidationRunRiskLevels:countByRiskLevel(toolValidationRuns),
    toolValidationRunStatuses:toolValidationRuns.reduce((counts, record) => {
      const status = record.status || "unknown";
      counts[status] = (counts[status] || 0) + 1;
      return counts;
    }, {}),
    invalidToolValidationRuns:toolValidationIssues.invalid,
    toolValidationRunQualityIssues:toolValidationIssues.byIssue,
    toolValidationRunQualityActions:toolValidationRunQualityActions(toolValidationRuns).slice(0, 50),
    runtimeGateEvents:runtimeGateEvents.length,
    runtimeGateEventReviewStatus:countByStatus(runtimeGateEvents),
    runtimeGateEventRiskLevels:countByRiskLevel(runtimeGateEvents),
    runtimeGateEventRouteModels:runtimeGateEvents.reduce((counts, record) => {
      const routeModel = record.route_model || "unknown";
      counts[routeModel] = (counts[routeModel] || 0) + 1;
      return counts;
    }, {}),
    runtimeGateEventBlockedReasons:runtimeGateEvents.reduce((counts, record) => {
      const reason = record.blocked_external_reason || "allowed";
      counts[reason] = (counts[reason] || 0) + 1;
      return counts;
    }, {}),
    runtimeGateExternalAllowed:runtimeGateEvents.filter(record => record.external_model_allowed === true).length,
    invalidRuntimeGateEvents:runtimeGateEventIssues.invalid,
    runtimeGateEventQualityIssues:runtimeGateEventIssues.byIssue,
    reviewQueue,
    reviewQueueItems,
...
function highRiskToolValidationReadiness(validationRuns = [], {
  toolId = "M4",
  minInternalValidationRuns = 3,
  maxFalseNegativeFindings = 0,
  requireExpertSignoff = true,
} = {}) {
  const normalizedToolId = `${toolId || ""}`.trim();
  const scopedRuns = validationRuns.filter(run => `${run.tool_id || run.toolId || ""}`.trim() === normalizedToolId);
  const approvedInternalRuns = scopedRuns.filter(run => {
    const mode = `${run.mode || run.release_mode || ""}`.trim();
    const status = `${run.status || run.run_status || ""}`.trim();
    return mode === "internal_pilot" && ["passed", "approved", "pass"].includes(status) && run.review_status === "approved";
  });
  const validationRunQuality = scopedRuns.map(run => ({ run, quality:validateToolValidationRunRecord(run) }));
  const invalidValidationRuns = validationRunQuality.filter(item => !item.quality.ok);
  const validationRunQualityIssues = invalidValidationRuns.reduce((counts, item) => {
    for (const issue of item.quality.issues) {
      counts[issue] = (counts[issue] || 0) + 1;
    }
    return counts;
  }, {});
  const falseNegativeFindings = scopedRuns.reduce((sum, run) => {
    const metadata = run.metadata && typeof run.metadata === "object" ? run.metadata : {};
    return sum + Number(run.false_negative_findings || metadata.false_negative_findings || 0);
  }, 0);
  const expertSignedRuns = approvedInternalRuns.filter(run => {
    const metadata = run.metadata && typeof run.metadata === "object" ? run.metadata : {};
    return `${metadata.reviewed_by || ""}`.trim() && `${metadata.reviewed_at || ""}`.trim();
  });
  const blockers = [];
  if (approvedInternalRuns.length < minInternalValidationRuns) {
    blockers.push({
      gate:"internal_validation_runs",
      current:approvedInternalRuns.length,
      required:minInternalValidationRuns,
    });
  }
  if (falseNegativeFindings > maxFalseNegativeFindings) {
    blockers.push({
      gate:"false_negative_findings",
      current:falseNegativeFindings,
      required:maxFalseNegativeFindings,
    });
  }
  if (invalidValidationRuns.length > 0) {
    blockers.push({
      gate:"validation_run_quality",
      current:invalidValidationRuns.length,
      required:0,
    });
  }
  if (requireExpertSignoff && expertSignedRuns.length < approvedInternalRuns.length) {
    blockers.push({
      gate:"expert_validation_signoff",
      current:expertSignedRuns.length,
      required:approvedInternalRuns.length,
    });
  }
  const actionByGate = {
    internal_validation_runs:"run_internal_pilot_validation_cases",
    false_negative_findings:"repair_false_negative_eval_failures",
    validation_run_quality:"repair_tool_validation_run_evidence",
    expert_validation_signoff:"record_expert_validation_signoff",
  };
  return {
    ready:blockers.length === 0,
    toolId:normalizedToolId,
    totalRuns:scopedRuns.length,
    approvedInternalRuns:approvedInternalRuns.length,
    invalidValidationRuns:invalidValidationRuns.length,
    validationRunQualityIssues,
    falseNegativeFindings,
    expertSignedRuns:expertSignedRuns.length,
    gates:{
      minInternalValidationRuns,
      maxFalseNegativeFindings,
      requireExpertSignoff,
    },
    blockers,
    actions:blockers.map(blocker => ({
      id:`high-risk-tool-validation-action:${blocker.gate}`,
      gate:blocker.gate,
      action:actionByGate[blocker.gate] || "manual_validation_review",
      current:blocker.current,
      required:blocker.required,
      blocksReadiness:true,
    })),
...
function knowledgeBrainHighRiskToolReadiness(input = {}, {
  ...
  coldStartOptions = {},
  evalMixOptions = {},
  falseNegativeEvalOptions = {},
  validationOptions = {},
  requireFalseNegativeEvalCoverage = externalRelease,
  requireInternalValidation = externalRelease,
  requireSourceContributionConsent = true,
} = {}) {
  const normalizedToolId = `${toolId || ""}`.trim();
  ...
  const falseNegativeEvalCoverage = evalCaseFalseNegativeCoverage(input.evalCases || [], {
    toolId:normalizedToolId,
    ...falseNegativeEvalOptions,
  });
  ...
  const contributionConsentActions = requireSourceContributionConsent
    ...
    : [];
  const toolValidation = highRiskToolValidationReadiness(input.toolValidationRuns || input.validationRuns || [], {
    toolId:normalizedToolId,
    ...validationOptions,
  });
  const blockers = [];
  ...
  if (requireFalseNegativeEvalCoverage && !falseNegativeEvalCoverage.ready) addNestedBlockers("false_negative_eval_set", falseNegativeEvalCoverage);
  if (requireInternalValidation && !toolValidation.ready) addNestedBlockers("internal_validation", toolValidation);
  if (requireSourceContributionConsent) {
  ...
  const actions = [
    ...
    ...(requireFalseNegativeEvalCoverage ? falseNegativeEvalCoverage.actions.map(action => ({ ...action, readinessGate:"false_negative_eval_set" })) : []),
    ...(requireInternalValidation ? toolValidation.actions.map(action => ({ ...action, readinessGate:"internal_validation" })) : []),
...
async function knowledgeStats() {
  ...
  const [docs, memories, archived, sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, calculationRuns, propertyDossiers, toolValidationRuns, runtimeGateEvents, ...jreRecordGroups] = await Promise.all([
    ...
    readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name),
    readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name),
    readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.runtimeGateEvents.name),
  ...
  return {
    ...knowledgeBrainInventoryStats({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns, propertyDossiers, toolValidationRuns, runtimeGateEvents }),
...
async function requestSourceDeletion(sourceId, { reason = "user_requested", requestedBy = "user" } = {}) {
  ...
  const touched = await new Promise((resolve, reject) => {
    const counts = { sources:0, knowledgeUnits:0, policyRules:0, scenarios:0, evalCases:0, evidenceRefs:0, japaneseRealEstateRecords:0, calculationRuns:0, toolValidationRuns:0 };
    const storeNames = [
      ...
      KNOWLEDGE_BRAIN_STORES.propertyDossiers.name,
      KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name,
    ...
    archiveLinkedRecord(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name, "propertyDossiers", item => Array.isArray(item.source_ids) && item.source_ids.includes(safeSourceId));
    archiveLinkedRecord(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name, "toolValidationRuns", item => Array.isArray(item.source_ids) && item.source_ids.includes(safeSourceId));
...
function buildPropertyDossierRecordFromWorkspace({
  propertyId,
  records = [],
  sources = [],
  evidenceRefs = [],
  calculationRuns = [],
  title = "",
  dossierType = "workspace_snapshot",
  reviewStatus = "candidate",
  riskLevel = "medium",
  metadata = {},
} = {}) {
  const dossier = buildPropertyDossier({ propertyId, records, sources, evidenceRefs });
  const scopedCalculations = calculationRuns.filter(run => run.property_id === propertyId);
  const sourceIds = [...new Set([
    ...dossier.byType.property.concat(...JRE_ENTITY_TYPES.filter(type => type !== "property").map(type => dossier.byType[type] || [])).map(record => record.source_id),
    ...scopedCalculations.flatMap(run => Array.isArray(run.source_ids) ? run.source_ids : []),
  ].filter(Boolean))];
  const evidenceRefIds = [...new Set([
    ...dossier.evidenceRefIds,
    ...scopedCalculations.flatMap(run => Array.isArray(run.evidence_ref_ids) ? run.evidence_ref_ids : []),
  ].filter(Boolean))];
  const jreRecordIds = JRE_ENTITY_TYPES.reduce((groups, type) => {
    const ids = (dossier.byType[type] || []).map(record => record.id).filter(Boolean);
    if (ids.length) groups[type] = ids;
    return groups;
  }, {});
  const unresolvedRiskIds = (dossier.byType.risk || [])
    .filter(record => record.review_status !== "approved" || record.requires_expert_confirmation !== true)
    .map(record => record.id)
    .filter(Boolean);
  const record = buildPropertyDossierRecord({
    property_id:propertyId,
    dossier_type:dossierType,
    title:title || `${propertyId} property dossier`,
    source_ids:sourceIds,
    evidence_ref_ids:evidenceRefIds,
    jre_record_ids:jreRecordIds,
    calculation_run_ids:scopedCalculations.map(run => run.id).filter(Boolean),
    summary_snapshot:{
      ...
      calculationOutputs:scopedCalculations.reduce((outputs, run) => {
        outputs[run.calculation_type || run.id] = run.outputs || {};
        return outputs;
      }, {}),
    },
    unresolved_risk_ids:unresolvedRiskIds,
    review_status:reviewStatus,
...
function buildToolValidationRunUpdatePayload(current = {}, patch = {}, options = {}) {
  const updatePatch = options.incrementVersion === false
    ? {
        ...patch,
        updated_at:options.now || new Date().toISOString(),
  ...
  const record = applyReviewDecisionMetadata({
    ...current,
    ...updatePatch,
    mode:updatePatch.mode || current.mode || "internal_pilot",
  }, options);
  const quality = validateToolValidationRunRecord(record);
  return {
    record,
    quality,
    validationReadiness:highRiskToolValidationReadiness([record], { toolId:record.tool_id }),
  };
}
...
function buildPropertyDossierUpdatePayload(current = {}, patch = {}, options = {}) {
  const updatePatch = options.incrementVersion === false
    ? {
        ...patch,
        updated_at:options.now || new Date().toISOString(),
  ...
  return {
    record,
    quality,
    ...recordReviewQueueContext("property_dossier", record),
  };
}

function filterPropertyDossierRecords(records = [], { propertyId = "", dossierType = "", sourceIds = [], statuses = [], reviewStatuses = [], riskLevels = [], includeArchived = false, query = "" } = {}) {
  const sourceIdSet = new Set(sourceIds);
  const reviewStatusSet = new Set([...statuses, ...reviewStatuses]);
  const riskLevelSet = new Set(riskLevels);
  const keyword = `${query || ""}`.trim().toLowerCase();
  return records
    .filter(record => includeArchived || record.review_status !== "archived")
    .filter(record => !propertyId || record.property_id === propertyId)
    .filter(record => !dossierType || record.dossier_type === dossierType)
    .filter(record => !sourceIdSet.size || (Array.isArray(record.source_ids) && record.source_ids.some(sourceId => sourceIdSet.has(sourceId))))
    .filter(record => !reviewStatusSet.size || reviewStatusSet.has(record.review_status))
    .filter(record => !riskLevelSet.size || riskLevelSet.has(record.risk_level))
    .filter(record => {
      ...
      return [
        ...
        ...(record.source_ids || []),
        ...(record.evidence_ref_ids || []),
        ...(record.calculation_run_ids || []),
        record.jre_record_ids ? JSON.stringify(record.jre_record_ids) : "",
        record.summary_snapshot ? JSON.stringify(record.summary_snapshot) : "",
        record.metadata ? JSON.stringify(record.metadata) : "",
      ].join("\n").toLowerCase().includes(keyword);
    })
    .sort((a, b) => (b.updated_at || "").localeCompare(a.updated_at || ""));
...
function filterToolValidationRunRecords(records = [], { toolId = "", toolIds = [], modes = [], statuses = [], reviewStatuses = [], riskLevels = [], sourceIds = [], evalCaseIds = [], includeArchived = false, query = "" } = {}) {
  const toolIdSet = new Set(toolIds);
  if (toolId) toolIdSet.add(toolId);
  const modeSet = new Set(modes);
  const statusSet = new Set(statuses);
  const reviewStatusSet = new Set(reviewStatuses);
  const riskLevelSet = new Set(riskLevels);
  const sourceIdSet = new Set(sourceIds);
  const evalCaseIdSet = new Set(evalCaseIds);
  const keyword = `${query || ""}`.trim().toLowerCase();
  return records
    .filter(record => includeArchived || record.review_status !== "archived")
    .filter(record => !toolIdSet.size || toolIdSet.has(record.tool_id))
    .filter(record => !modeSet.size || modeSet.has(record.mode))
    .filter(record => !statusSet.size || statusSet.has(record.status))
    .filter(record => !reviewStatusSet.size || reviewStatusSet.has(record.review_status))
    .filter(record => !riskLevelSet.size || riskLevelSet.has(record.risk_level))
    .filter(record => !sourceIdSet.size || (Array.isArray(record.source_ids) && record.source_ids.some(sourceId => sourceIdSet.has(sourceId))))
    .filter(record => !evalCaseIdSet.size || (Array.isArray(record.eval_case_ids) && record.eval_case_ids.some(evalCaseId => evalCaseIdSet.has(evalCaseId))))
    .filter(record => {
      if (!keyword) return true;
      return [
        record.id,
        ...
        record.false_negative_findings,
        ...(record.eval_case_ids || []),
        ...(record.source_ids || []),
        ...(record.evidence_ref_ids || []),
        record.findings ? JSON.stringify(record.findings) : "",
        record.metadata ? JSON.stringify(record.metadata) : "",
      ].join("\n").toLowerCase().includes(keyword);
    })
    .sort((a, b) => (b.updated_at || "").localeCompare(a.updated_at || ""));
...
function filterRuntimeGateEventRecords(records = [], { toolId = "", toolIds = [], actions = [], routeModels = [], externalAllowed = null, blockedReasons = [], reviewStatuses = [], riskLevels = [], sourceIds = [], knowledgeIds = [], includeArchived = false, query = "" } = {}) {
  const toolIdSet = new Set(toolIds);
  if (toolId) toolIdSet.add(toolId);
  const actionSet = new Set(actions);
  const routeModelSet = new Set(routeModels);
  const blockedReasonSet = new Set(blockedReasons);
  const reviewStatusSet = new Set(reviewStatuses);
  const riskLevelSet = new Set(riskLevels);
  const sourceIdSet = new Set(sourceIds);
  const knowledgeIdSet = new Set(knowledgeIds);
  const hasExternalAllowedFilter = typeof externalAllowed === "boolean";
  const keyword = `${query || ""}`.trim().toLowerCase();
  return records
    .filter(record => includeArchived || record.review_status !== "archived")
    .filter(record => !toolIdSet.size || toolIdSet.has(record.tool_id))
    .filter(record => !actionSet.size || actionSet.has(record.action))
    .filter(record => !routeModelSet.size || routeModelSet.has(record.route_model))
    .filter(record => !hasExternalAllowedFilter || record.external_model_allowed === externalAllowed)
    .filter(record => !blockedReasonSet.size || blockedReasonSet.has(record.blocked_external_reason))
    .filter(record => !reviewStatusSet.size || reviewStatusSet.has(record.review_status))
    .filter(record => !riskLevelSet.size || riskLevelSet.has(record.risk_level))
    .filter(record => !sourceIdSet.size || (Array.isArray(record.source_ids) && record.source_ids.some(sourceId => sourceIdSet.has(sourceId))))
    .filter(record => !knowledgeIdSet.size || (Array.isArray(record.knowledge_ids) && record.knowledge_ids.some(knowledgeId => knowledgeIdSet.has(knowledgeId))))
    .filter(record => {
      if (!keyword) return true;
      return [
        record.id,
        ...
        record.task_type,
        record.route_model,
        record.blocked_external_reason,
        ...(record.source_ids || []),
        ...(record.knowledge_ids || []),
        record.policy_result ? JSON.stringify(record.policy_result) : "",
        record.output_quality ? JSON.stringify(record.output_quality) : "",
        record.metadata ? JSON.stringify(record.metadata) : "",
      ].join("\n").toLowerCase().includes(keyword);
    })
...
async function putPropertyDossier(input = {}) {
  const record = input.property_id && input.created_at ? input : buildPropertyDossierRecord(input);
  const quality = validatePropertyDossierRecord(record);
  const db = await openKnowledgeDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name, "readwrite");
    tx.objectStore(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name).put(record);
    tx.oncomplete = resolve;
    tx.onerror = () => reject(tx.error);
  });
  db.close();
...
async function updatePropertyDossier(id, patch = {}, options = {}) {
  const safeId = `${id || ""}`.trim();
  if (!safeId) throw new Error("id is required.");
  const db = await openKnowledgeDb();
  const result = await new Promise((resolve, reject) => {
    let output = null;
    const tx = db.transaction(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name, "readwrite");
    const store = tx.objectStore(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name);
    const request = store.get(safeId);
    request.onsuccess = () => {
      const current = request.result;
      if (!current) return;
      const payload = buildPropertyDossierUpdatePayload(current, patch, options);
      store.put(payload.record);
      output = payload;
    };
    request.onerror = () => reject(request.error);
...
async function listPropertyDossiers(filters = {}) {
  const records = await readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name);
  return filterPropertyDossierRecords(records, filters);
}
...
async function putToolValidationRun(input = {}) {
  const record = input.tool_id && input.created_at ? input : buildToolValidationRunRecord(input);
  const quality = validateToolValidationRunRecord(record);
  const db = await openKnowledgeDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name, "readwrite");
    tx.objectStore(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name).put(record);
    tx.oncomplete = resolve;
    tx.onerror = () => reject(tx.error);
  });
  db.close();
...
async function updateToolValidationRun(id, patch = {}, options = {}) {
  const safeId = `${id || ""}`.trim();
  if (!safeId) throw new Error("id is required.");
  const db = await openKnowledgeDb();
  const result = await new Promise((resolve, reject) => {
    let output = null;
    const tx = db.transaction(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name, "readwrite");
    const store = tx.objectStore(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name);
    const request = store.get(safeId);
    request.onsuccess = () => {
      const current = request.result;
      if (!current) return;
      const payload = buildToolValidationRunUpdatePayload(current, patch, options);
      store.put(payload.record);
      output = payload;
    };
    request.onerror = () => reject(request.error);
...
async function listToolValidationRuns(filters = {}) {
  const records = await readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.toolValidationRuns.name);
  return filterToolValidationRunRecords(records, filters);
}

async function putRuntimeGateEvent(input = {}) {
  const record = input.tool_id && input.created_at ? input : buildRuntimeGateEventRecord(input);
  const quality = validateRuntimeGateEventRecord(record);
  const db = await openKnowledgeDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(KNOWLEDGE_BRAIN_STORES.runtimeGateEvents.name, "readwrite");
    tx.objectStore(KNOWLEDGE_BRAIN_STORES.runtimeGateEvents.name).put(record);
    tx.oncomplete = resolve;
    tx.onerror = () => reject(tx.error);
  });
  db.close();
...
async function listRuntimeGateEvents(filters = {}) {
  const records = await readKnowledgeBrainStore(KNOWLEDGE_BRAIN_STORES.runtimeGateEvents.name);
  return filterRuntimeGateEventRecords(records, filters);
}
...
async function searchKnowledge(query) {
  ...
  const [docs, chunks, sources, units, evidenceRefs, policyRules, scenarios, evalCases, calculationRuns, propertyDossiers, ...jreRecordGroups] = await Promise.all([
    ...
    readStoreFromDb(KNOWLEDGE_BRAIN_STORES.propertyDossiers.name),
...
function knowledgePanelSearchResults({ query, documents = [], chunks = [], sources = [], units = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], propertyDossiers = [], limit = 12 } = {}) {
  const keyword = `${query || ""}`.trim().toLowerCase();
  if (!keyword) return [];
...
async function searchKnowledgeForPanel(query) {
  ...
  const [documents, chunks, sources, units, evidenceRefs, policyRules, scenarios, evalCases, calculationRuns, propertyDossiers, ...jreRecordGroups] = await Promise.all([
...
const OUTPUT_MODEL_MODES = Object.freeze(["template_only", "knowledge_only", "small_model", "large_model"]);
const OUTPUT_RISK_LEVELS = Object.freeze(["low", "medium", "high", "critical"]);

const POLICY_ENGINE_RULES = Object.freeze({
  P001:{ id:"P001", appliesTo:["any"], riskLevel:"low", requiresSources:true, requiresKakunin:false, requiresDisclaimer:false },
  P002:{ id:"P002", appliesTo:["valuation", "market_price", "asset_value", "price"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:false },
  P003:{ id:"P003", appliesTo:["contract", "important_matter", "legal"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
  P004:{ id:"P004", appliesTo:["tax", "gift", "inheritance"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
  P005:{ id:"P005", appliesTo:["loan", "mortgage", "screening"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
  P006:{ id:"P006", appliesTo:["structure", "earthquake", "seismic"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
  P007:{ id:"P007", appliesTo:["hazard", "disaster", "flood"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:false },
  P008:{ id:"P008", appliesTo:["feng_shui", "culture"], riskLevel:"medium", requiresSources:false, requiresKakunin:false, requiresDisclaimer:true },
  P009:{ id:"P009", appliesTo:["repair_estimate", "construction_cost"], riskLevel:"medium", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
  P010:{ id:"P010", appliesTo:["school", "neighborhood", "transport"], riskLevel:"medium", requiresSources:true, requiresKakunin:true, requiresDisclaimer:false },
  P011:{ id:"P011", appliesTo:["foreign_customer", "translation"], riskLevel:"high", requiresSources:true, requiresKakunin:true, requiresDisclaimer:true },
});

const POLICY_KEYWORDS = Object.freeze({
  P002:[/価格|市場価値|査定|相場|資産価値|valuation|market price|asset value|price/i],
  P003:[/契約|重要事項|重説|法律|legal|contract|important matter/i],
  P004:[/税務|贈与|相続|税額|節税|tax|gift|inheritance/i],
  P005:[/ローン|融資|審査|金融機関|mortgage|loan|screening/i],
  P006:[/建築|構造|耐震|安全性|structure|seismic|earthquake/i],
  P007:[/ハザード|災害|洪水|浸水|土砂|hazard|disaster|flood/i],
  P008:[/風水|方位|文化|feng shui/i],
  P009:[/施工|見積|工事費|repair|construction cost|estimate/i],
  P010:[/学区|周辺|交通|駅徒歩|school|neighborhood|transport/i],
  P011:[/外国人|翻訳|多言語|foreign customer|translation/i],
});

const HIGH_RISK_FINAL_CLAIM_PATTERNS = Object.freeze([
  /必ず|絶対|保証|断定|問題ない|安全です|違法ではない|承認されます|節税できます/,
  /guarantee|definitely|safe|no risk|will be approved|final legal|final tax/i,
  /一定|绝对|保证|没有风险|最终结论|肯定获批/,
]);

const KNOWLEDGE_BRAIN_TOOL_REGISTRY = Object.freeze({
  M1:{ tool_id:"M1", name:"property_summary", risk_level:"medium", required_domains:["D01", "D02"], required_policy_ids:["P001"], allowed_model_modes:["template_only", "knowledge_only", "small_model"], output_template:"property_summary", requires_review:false },
  M2:{ tool_id:"M2", name:"customer_reply", risk_level:"medium", required_domains:["D03", "D10"], required_policy_ids:["P001", "P011"], allowed_model_modes:["template_only", "knowledge_only", "small_model"], output_template:"customer_reply", requires_review:false },
  M3:{ tool_id:"M3", name:"area_explanation", risk_level:"medium", required_domains:["D09", "D10"], required_policy_ids:["P001", "P010"], allowed_model_modes:["template_only", "knowledge_only", "small_model"], output_template:"area_explanation", requires_review:false },
  M4:{ tool_id:"M4", name:"valuation_rationale", risk_level:"high", required_domains:["D04", "D05", "D06"], required_policy_ids:["P001", "P002"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"valuation_rationale_review_candidate", requires_review:true, internal_assistive_only:true },
  M5:{ tool_id:"M5", name:"contract_risk_check", risk_level:"high", required_domains:["D07", "D08", "D16"], required_policy_ids:["P001", "P003"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"contract_risk_review_candidate", requires_review:true, internal_assistive_only:true },
  M6:{ tool_id:"M6", name:"loan_explanation", risk_level:"high", required_domains:["D08"], required_policy_ids:["P001", "P005"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"loan_explanation", requires_review:true },
  M7:{ tool_id:"M7", name:"tax_boundary_note", risk_level:"high", required_domains:["D08"], required_policy_ids:["P001", "P004"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"tax_boundary_note", requires_review:true },
  M8:{ tool_id:"M8", name:"hazard_explanation", risk_level:"high", required_domains:["D08", "D16"], required_policy_ids:["P001", "P007"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"hazard_explanation", requires_review:true },
  M9:{ tool_id:"M9", name:"repair_estimate_note", risk_level:"medium", required_domains:["D09"], required_policy_ids:["P001", "P009"], allowed_model_modes:["template_only", "knowledge_only", "small_model"], output_template:"repair_estimate_note", requires_review:false },
  M10:{ tool_id:"M10", name:"foreign_customer_explanation", risk_level:"high", required_domains:["D10", "D11"], required_policy_ids:["P001", "P011"], allowed_model_modes:["template_only", "knowledge_only"], output_template:"foreign_customer_explanation", requires_review:true },
});

function normalizeOutputRiskLevel(riskLevel = "medium") {
  const value = `${riskLevel || "medium"}`.trim().toLowerCase();
  return OUTPUT_RISK_LEVELS.includes(value) ? value : "medium";
}

function policyRuleIdsForTask({ taskType = "", prompt = "", riskLevel = "", domains = [] } = {}) {
  const haystack = [taskType, prompt, ...(domains || [])].join("\n");
  const ids = new Set(["P001"]);
  for (const [ruleId, patterns] of Object.entries(POLICY_KEYWORDS)) {
    if (patterns.some(pattern => pattern.test(haystack))) ids.add(ruleId);
  }
  if (["high", "critical"].includes(normalizeOutputRiskLevel(riskLevel))) {
    if (!ids.has("P003") && /contract|契約|重要事項|重説/i.test(haystack)) ids.add("P003");
  }
  return [...ids];
}

function approvedPolicyRuleIds(policyRules = []) {
  return new Set(policyRules
    .filter(rule => rule?.review_status === "approved")
    .flatMap(rule => [rule.policy_id, rule.rule_id, rule.rule_type, rule.id, ...(Array.isArray(rule.applies_to) ? rule.applies_to : [])])
    .filter(Boolean));
}

function evaluatePolicyEngine({ taskType = "", prompt = "", riskLevel = "medium", sources = [], knowledgeIds = [], policyRules = [], domains = [], requireApprovedPolicyRules = true } = {}) {
  const normalizedRisk = normalizeOutputRiskLevel(riskLevel);
  const ruleIds = policyRuleIdsForTask({ taskType, prompt, riskLevel:normalizedRisk, domains });
  const approvedPolicyIds = approvedPolicyRuleIds(policyRules);
  const issues = [];
  const requiredKakuninItems = [];
  let requiresSources = false;
  let requiresDisclaimer = ["high", "critical"].includes(normalizedRisk);
  let blocksFinalAnswer = false;

  for (const ruleId of ruleIds) {
    const rule = POLICY_ENGINE_RULES[ruleId];
    if (!rule) continue;
    requiresSources = requiresSources || rule.requiresSources;
    requiresDisclaimer = requiresDisclaimer || rule.requiresDisclaimer;
    if (rule.requiresKakunin) requiredKakuninItems.push(`${ruleId}: 要確認`);
    if (requireApprovedPolicyRules && !approvedPolicyIds.has(ruleId)) {
      issues.push({ issue:"missing_approved_policy_rule", policy_rule_id:ruleId });
      blocksFinalAnswer = true;
    }
  }

  if (requiresSources && sources.length === 0 && knowledgeIds.length === 0) {
    issues.push({ issue:"missing_source_citation" });
    requiredKakuninItems.push("source evidence missing");
    blocksFinalAnswer = true;
  }
  if (ruleIds.includes("P002") && sources.length === 0) {
    issues.push({ issue:"source_less_market_value_claim_blocked" });
    blocksFinalAnswer = true;
  }
  if (HIGH_RISK_FINAL_CLAIM_PATTERNS.some(pattern => pattern.test(`${prompt || ""}`)) && ["high", "critical"].includes(normalizedRisk)) {
    issues.push({ issue:"unsafe_final_claim_requires_confirmation" });
    requiredKakuninItems.push("expert confirmation required");
    blocksFinalAnswer = true;
  }
  ...
  return {
    ...
    required_kakunin_items:[...new Set(requiredKakuninItems)],
    blocks_final_answer:blocksFinalAnswer,
    issues,
    actions:issues.map(issue => ({
      ...issue,
      action:issue.issue === "missing_approved_policy_rule"
        ? "approve_policy_rule_before_runtime"
        : issue.issue === "missing_source_citation" || issue.issue === "source_less_market_value_claim_blocked"
          ? "attach_approved_source_or_downgrade_to_kakunin"
          : "downgrade_to_review_candidate",
      blocks_final_answer:true,
    })),
  };
}

function defaultKnowledgeBrainDisclaimer(riskLevel = "medium") {
  return ["high", "critical"].includes(normalizeOutputRiskLevel(riskLevel))
    ? "This output is a source-backed review candidate and must be confirmed by a qualified professional before final use."
    : "";
}

function buildOutputBuilderPayload({ answerBody = "", sources = [], kakuninItems = [], disclaimer = "", modelUsed = "knowledge_only", knowledgeIdsCited = [], riskLevel = "medium" } = {}, { policyResult = null } = {}) {
  const normalizedRisk = normalizeOutputRiskLevel(policyResult?.risk_level || riskLevel);
  const normalizedModel = OUTPUT_MODEL_MODES.includes(modelUsed) ? modelUsed : "knowledge_only";
  const requiredKakunin = Array.isArray(policyResult?.required_kakunin_items) ? policyResult.required_kakunin_items : [];
  const outputDisclaimer = disclaimer || (policyResult?.requires_disclaimer ? defaultKnowledgeBrainDisclaimer(normalizedRisk) : "");
  return {
    answer_body:`${answerBody || ""}`.trim(),
    sources:[...new Set((Array.isArray(sources) ? sources : []).filter(Boolean))],
    kakunin_items:[...new Set([...(Array.isArray(kakuninItems) ? kakuninItems : []), ...requiredKakunin].filter(Boolean))],
    disclaimer:outputDisclaimer,
    model_used:normalizedModel,
    knowledge_ids_cited:[...new Set((Array.isArray(knowledgeIdsCited) ? knowledgeIdsCited : []).filter(Boolean))],
    risk_level:normalizedRisk,
  };
}

function validateOutputBuilderPayload(payload = {}) {
  const issues = [];
  if (!`${payload.answer_body || ""}`.trim()) issues.push("missing_answer_body");
  if (!Array.isArray(payload.sources)) issues.push("invalid_sources");
  if (!Array.isArray(payload.kakunin_items)) issues.push("invalid_kakunin_items");
  if (!OUTPUT_MODEL_MODES.includes(payload.model_used)) issues.push("invalid_model_used");
  if (!Array.isArray(payload.knowledge_ids_cited)) issues.push("invalid_knowledge_ids_cited");
  if (!OUTPUT_RISK_LEVELS.includes(payload.risk_level)) issues.push("invalid_risk_level");
  if (["high", "critical"].includes(payload.risk_level) && !`${payload.disclaimer || ""}`.trim()) issues.push("high_risk_missing_disclaimer");
  if (["high", "critical"].includes(payload.risk_level) && (!Array.isArray(payload.kakunin_items) || payload.kakunin_items.length === 0)) issues.push("high_risk_missing_kakunin_items");
  if ((!Array.isArray(payload.sources) || payload.sources.length === 0) && (!Array.isArray(payload.knowledge_ids_cited) || payload.knowledge_ids_cited.length === 0)) issues.push("missing_sources_or_knowledge_ids");
  return { ok:issues.length === 0, issues };
}

function buildKnowledgeTemplateRecord({ id = "", templateId = "", title = "", body = "", requiredFields = [], languageMode = "ja", sourceId = "", knowledgeIds = [], policyRuleIds = [], reviewStatus = "candidate", riskLevel = "medium", version = 1, metadata = {} } = {}) {
  return {
    id:id || templateId || `template-${Date.now()}`,
    template_id:templateId || id || "",
    title,
    body,
    required_fields:[...new Set((Array.isArray(requiredFields) ? requiredFields : []).filter(Boolean))],
    language_mode:languageMode,
    source_id:sourceId,
    knowledge_ids:[...new Set((Array.isArray(knowledgeIds) ? knowledgeIds : []).filter(Boolean))],
    policy_rule_ids:[...new Set((Array.isArray(policyRuleIds) ? policyRuleIds : []).filter(Boolean))],
    review_status:reviewStatus,
    risk_level:normalizeOutputRiskLevel(riskLevel),
    version:Math.max(1, Number(version) || 1),
    metadata:metadata && typeof metadata === "object" ? metadata : {},
  };
}

function renderKnowledgeTemplate(template = {}, inputs = {}, options = {}) {
  const missingFields = (Array.isArray(template.required_fields) ? template.required_fields : [])
    .filter(field => !`${inputs?.[field] ?? ""}`.trim());
  const answerBody = `${template.body || ""}`.replace(/\{\{\s*([\w.-]+)\s*\}\}/g, (_, field) => `${inputs?.[field] ?? ""}`.trim());
  const policyResult = options.policyResult || evaluatePolicyEngine({
    taskType:template.template_id || template.title,
    prompt:answerBody,
    riskLevel:template.risk_level,
    sources:options.sources || (template.source_id ? [template.source_id] : []),
    knowledgeIds:template.knowledge_ids || [],
    policyRules:options.policyRules || [],
    requireApprovedPolicyRules:options.requireApprovedPolicyRules === true,
  });
  return {
    output:buildOutputBuilderPayload({
      answerBody,
      sources:options.sources || (template.source_id ? [template.source_id] : []),
      kakuninItems:missingFields.map(field => `${field}: missing input`),
      disclaimer:options.disclaimer || "",
      modelUsed:"template_only",
      knowledgeIdsCited:template.knowledge_ids || [],
      riskLevel:template.risk_level,
    }, { policyResult }),
...
function routeKnowledgeBrainModel({ taskType = "", riskLevel = "medium", hasTemplate = false, hasApprovedKnowledge = false, localOnly = false, auditOnly = false, requestedModelMode = "" } = {}) {
  if (localOnly || auditOnly) {
    return { model_used:hasTemplate ? "template_only" : hasApprovedKnowledge ? "knowledge_only" : "template_only", external_model_allowed:false, blocked_external_reason:localOnly ? "local_only" : "audit_only" };
  }
  if (requestedModelMode && OUTPUT_MODEL_MODES.includes(requestedModelMode)) {
    return { model_used:requestedModelMode, external_model_allowed:["small_model", "large_model"].includes(requestedModelMode), blocked_external_reason:"" };
  }
  const normalizedRisk = normalizeOutputRiskLevel(riskLevel);
  if (hasTemplate) return { model_used:"template_only", external_model_allowed:false, blocked_external_reason:"" };
  if (hasApprovedKnowledge || ["high", "critical"].includes(normalizedRisk)) return { model_used:"knowledge_only", external_model_allowed:false, blocked_external_reason:"" };
  return { model_used:taskType === "draft" ? "small_model" : "knowledge_only", external_model_allowed:taskType === "draft", blocked_external_reason:"" };
}

function knowledgeBrainToolRegistry() {
  return Object.values(KNOWLEDGE_BRAIN_TOOL_REGISTRY).map(tool => ({ ...tool }));
}

function knowledgeBrainToolRuntimeGate(toolId = "", { toolValidationRuns = [], evalCases = [], externalRelease = false } = {}) {
  const tool = KNOWLEDGE_BRAIN_TOOL_REGISTRY[toolId];
  if (!tool) return { ok:false, tool_id:toolId, issues:["unknown_tool"], actions:["register_tool_before_runtime"] };
  const issues = [];
  if (!tool.required_policy_ids?.includes("P001")) issues.push("missing_source_policy_gate");
  if (externalRelease && tool.internal_assistive_only) issues.push("tool_internal_assistive_only");
  if (["M4", "M5"].includes(toolId)) {
    const readiness = highRiskToolValidationReadiness(toolValidationRuns, { toolId });
    const coverage = evalCaseFalseNegativeCoverage(evalCases, { toolId });
    if (!readiness.ready) issues.push("high_risk_validation_not_ready");
    if (!coverage.ready) issues.push("false_negative_eval_coverage_not_ready");
  }
  return {
    ok:issues.length === 0,
    tool_id:toolId,
    risk_level:tool.risk_level,
    allowed_model_modes:tool.allowed_model_modes,
    internal_assistive_only:tool.internal_assistive_only === true,
    issues,
    actions:issues.map(issue => issue === "tool_internal_assistive_only"
      ? "keep_tool_internal_until_human_release"
      : issue === "missing_source_policy_gate"
        ? "attach_p001_source_policy_gate"
        : "complete_high_risk_tool_validation"),
  };
}

function buildKnowledgeBrainRuntimeResult({ toolId = "", taskType = "", prompt = "", languageMode = "ja", riskLevel = "medium", retrievalResults = [], policyRules = [], template = null, templateInputs = {}, answerBody = "", localOnly = false, auditOnly = false } = {}) {
  const tool = KNOWLEDGE_BRAIN_TOOL_REGISTRY[toolId] || null;
  const sources = [...new Set(retrievalResults.flatMap(item => [item.sourceId, ...(item.sourceIds || [])]).filter(Boolean))];
  const knowledgeIds = [...new Set(retrievalResults.map(item => item.id).filter(Boolean))];
  const effectiveRisk = normalizeOutputRiskLevel(riskLevel || tool?.risk_level || "medium");
  const policy = evaluatePolicyEngine({
    taskType:taskType || tool?.name || toolId,
    prompt,
    riskLevel:effectiveRisk,
    sources,
    knowledgeIds,
    policyRules,
    domains:tool?.required_domains || [],
    requireApprovedPolicyRules:policyRules.length > 0,
  });
  const route = routeKnowledgeBrainModel({
    taskType,
    riskLevel:effectiveRisk,
    hasTemplate:!!template,
    hasApprovedKnowledge:retrievalResults.length > 0,
    localOnly,
    auditOnly,
  });
  const rendered = template
    ? renderKnowledgeTemplate(template, templateInputs, { policyResult:policy, sources, requireApprovedPolicyRules:false })
    : null;
  const output = rendered?.output || buildOutputBuilderPayload({
    answerBody:answerBody || prompt,
    sources,
    kakuninItems:policy.blocks_final_answer ? ["review required before final use"] : [],
    modelUsed:route.model_used,
    knowledgeIdsCited:knowledgeIds,
    riskLevel:effectiveRisk,
  }, { policyResult:policy });
  const outputQuality = validateOutputBuilderPayload(output);
  return {
    ok:policy.ok && outputQuality.ok && route.blocked_external_reason === "",
    tool_id:toolId,
    language_mode:languageMode,
    ...
    audit:{
      used_knowledge_brain:true,
      direct_model_prompt:false,
      source_ids:sources,
...
function buildHandoffPackage({ packageId = "", createdAt = new Date().toISOString().slice(0, 10), sourceSessionIds = [], summary = "", openKakuninItems = [], riskResiduals = [], nextActions = [], targetAgentOrTool = "", status = "pending" } = {}) {
  return {
    package_id:packageId || `HO-${createdAt.replace(/-/g, "")}-001`,
    created_at:createdAt,
    source_session_ids:Array.isArray(sourceSessionIds) ? sourceSessionIds : [],
    summary:`${summary || ""}`.trim(),
    open_kakunin_items:Array.isArray(openKakuninItems) ? openKakuninItems : [],
    risk_residuals:Array.isArray(riskResiduals) ? riskResiduals : [],
    next_actions:Array.isArray(nextActions) ? nextActions : [],
    target_agent_or_tool:`${targetAgentOrTool || ""}`.trim(),
    status:["pending", "accepted", "completed"].includes(status) ? status : "pending",
  };
}

function validateHandoffPackage(pkg = {}) {
  const issues = [];
  if (!`${pkg.package_id || ""}`.trim()) issues.push("missing_package_id");
  if (!`${pkg.created_at || ""}`.trim()) issues.push("missing_created_at");
  if (!Array.isArray(pkg.source_session_ids)) issues.push("invalid_source_session_ids");
  if (!`${pkg.summary || ""}`.trim()) issues.push("missing_summary");
  if (!Array.isArray(pkg.open_kakunin_items)) issues.push("invalid_open_kakunin_items");
  if (!Array.isArray(pkg.risk_residuals)) issues.push("invalid_risk_residuals");
  if (!Array.isArray(pkg.next_actions)) issues.push("invalid_next_actions");
  if (!`${pkg.target_agent_or_tool || ""}`.trim()) issues.push("missing_target_agent_or_tool");
  if (!["pending", "accepted", "completed"].includes(pkg.status)) issues.push("invalid_status");
  return { ok:issues.length === 0, issues };
}

const DEFAULT_REDACTION_RULES = Object.freeze([
  { id:"email", placeholder:"[EMAIL]", pattern:/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi },
  { id:"phone", placeholder:"[PHONE]", pattern:/(?:\+?\d{1,3}[-\s]?)?(?:0\d{1,4}[-\s]?\d{1,4}[-\s]?\d{3,4}|\d{2,4}-\d{2,4}-\d{3,4})/g },
  { id:"postal_code", placeholder:"[POSTAL_CODE]", pattern:/〒?\d{3}-?\d{4}/g },
  { id:"address", placeholder:"[ADDRESS]", pattern:/(?:東京都|北海道|(?:京都|大阪)府|.{2,3}県).{2,40}(?:市|区|町|村).{0,40}/g },
  { id:"person_name", placeholder:"[NAME]", pattern:/(?:氏名|名前|客户名|顧客名|お名前)[:：]\s*[^\s,，。]{2,20}/g },
]);

function redactionRulesForTier({ tier = "free", customRules = [] } = {}) {
  const normalizedTier = `${tier || "free"}`.trim().toLowerCase();
  const baseRules = normalizedTier === "free" ? DEFAULT_REDACTION_RULES : [];
  const normalizedCustomRules = (Array.isArray(customRules) ? customRules : [])
    .filter(rule => rule?.id && rule?.pattern && rule?.placeholder)
    .map(rule => ({
      id:`${rule.id}`,
      placeholder:`${rule.placeholder}`,
      pattern:rule.pattern instanceof RegExp ? rule.pattern : new RegExp(`${rule.pattern}`, "g"),
    }));
  return [...baseRules, ...normalizedCustomRules];
}

function redactUserBusinessData(text = "", options = {}) {
  let redacted = `${text || ""}`;
  const appliedRules = [];
  for (const rule of redactionRulesForTier(options)) {
    const before = redacted;
    redacted = redacted.replace(rule.pattern, rule.placeholder);
    if (before !== redacted) appliedRules.push(rule.id);
  }
  return {
    text:redacted,
    applied_rules:[...new Set(appliedRules)],
    redacted:appliedRules.length > 0,
  };
}

function userBusinessDataTrainingBoundary({ tier = "free", source = {}, text = "", customRedactionRules = [] } = {}) {
  const normalizedTier = `${tier || "free"}`.trim().toLowerCase();
  const sourceReport = sourceUsagePermissions(source);
  const redaction = redactUserBusinessData(text, { tier:normalizedTier, customRules:customRedactionRules });
  const trainingEligible = normalizedTier === "free"
    && sourceReport.training.allowed === true
    && redaction.text !== text
    && source.training_allowed === true;
  const reasons = [];
  if (normalizedTier !== "free") reasons.push("paid_tier_private_by_default");
  if (sourceReport.training.allowed !== true) reasons.push(...sourceReport.training.reasons);
  if (normalizedTier === "free" && redaction.text === text) reasons.push("no_redaction_applied");
  if (source.training_allowed !== true) reasons.push("training_not_enabled");
  return {
    tier:normalizedTier,
    local_only_default:true,
    training_allowed_after_redaction:trainingEligible,
    redaction,
    reasons:[...new Set(reasons)],
  };
}
...
export {
  ...
  buildPropertyDossierRecordFromWorkspace,
  buildPropertyDossierUpdatePayload,
  buildSourceRegistryIngestPayload,
  buildSourceRegistryUpdatePayload,
  buildSourceWithdrawalPatch,
  buildToolValidationRunUpdatePayload,
  ...
  filterPropertyDossierRecords,
  filterRuntimeGateEventRecords,
  ...
  filterSourceReferenceSearchEligibilityReport,
  filterToolValidationRunQualityActions,
  filterToolValidationRunRecords,
  filterProjectMemoriesBySourceType,
  findMemoryConflict,
  evaluatePolicyEngine,
  highRiskToolValidationReadiness,
  ...
  listPropertyDossiers,
  listKnowledgeGovernanceRecords,
  listKnowledgeDocuments,
  listKnowledgeUnits,
  listProjectMemories,
  listRuntimeGateEvents,
  listSourceRegistry,
  listToolValidationRuns,
  ...
  sourceReferenceSearchEligibilityActions,
  sourceReferenceSearchEligibilityReport,
  sourceDeletionImpactSummary,
  toolValidationRunQualityActions,
