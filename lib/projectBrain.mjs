...
function knowledgeBrainInventoryStats({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [] } = {}) {
  ...
  return {
    ...
    reviewerRoleSummary:knowledgeBrainReviewerRoleSummary({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns }),
...
function knowledgeBrainColdStartReadiness(input = {}, {
  ...
  const actions = knowledgeBrainColdStartReadinessActions(blockers);
  return {
    ready: blockers.length === 0,
