...
function sourceDeletionImpactSummary({ sourceId = "", sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], toolValidationRuns = [] } = {}) {
...
function knowledgeBrainExportManifest(payload = {}) {
  ...
  const manifest = brainStores.reduce((manifest, store) => {
    ...
    const storeSummary = items.reduce((summary, item = {}) => {
      ...
      const usesSourceIds = ["calculation_runs", "tool_validation_runs"].includes(store.name);
...
function validateKnowledgeBrainReferenceIntegrity({ sources = [], evidenceRefs = [], knowledgeUnits = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], toolValidationRuns = [] } = {}) {
...
function knowledgeBrainInventoryStats({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], toolValidationRuns = [] } = {}) {
...
async function knowledgeStats() {
  ...
  const [docs, memories, archived, sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, calculationRuns, toolValidationRuns, ...jreRecordGroups] = await Promise.all([
  ...
  return {
    ...knowledgeBrainInventoryStats({ sources, knowledgeUnits, evidenceRefs, policyRules, scenarios, evalCases, japaneseRealEstateRecords, calculationRuns, toolValidationRuns }),
