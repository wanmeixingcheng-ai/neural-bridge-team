...
function approvedKnowledgeBrainSearchResults({ query, sources = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [] } = {}) {
  ...
  const buildHit = ({ type, id, sourceId = "", sourceIds = [], title = "", content = "", evidenceRefIds = [], reviewStatus = "", riskLevel = "", version = 1 }) => {
    ...
    return { type, id, sourceId, sourceIds, title, content, evidenceRefIds, reviewStatus, riskLevel, version, score };
  };
...
function knowledgeBrainReviewQueueItems({ sources = [], knowledgeUnits = [], evidenceRefs = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], limit = 100, targetTypes = [], sourceIds = [], reviewStatuses = [], riskLevels = [], reasons = [], query = "" } = {}) {
  ...
  const addItem = ({ target_type, target_id, review_status, risk_level, source_id = "", source_ids = [], reasons = [], quality = null }) => {
