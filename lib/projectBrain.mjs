...
function sourceContributionConsentReport(sources = []) {
  return sources.reduce((report, source) => {
    ...
    if (contributorTier === "free_tier" && !["opt_in", "explicit_opt_in"].includes(consentScope)) report.freeTierMissingOptIn += 1;
    if (source.training_allowed === true) report.trainingAllowedSources += 1;
    if (source.training_allowed === true && consentScope !== "explicit_opt_in") report.trainingAllowedWithoutExplicitConsent += 1;
    if (source.deletion_requested === true) report.deletionRequestedSources += 1;
    if (metadata.deletion_requested_at || metadata.training_withdrawn_at) report.withdrawalAuditRecords += 1;
    if (source.deletion_requested === true && !metadata.deletion_requested_at) report.deletionRequestsMissingAudit += 1;
    return report;
  }, {
    total:0,
    freeTierSources:0,
...
function knowledgeBrainImportSizeSummary(payload = {}) {
  ...
  const totalTextChars = items.reduce((sum, item) => sum + `${item?.title || ""}${item?.content || ""}${item?.summary || ""}`.length, 0);
...
function knowledgeBrainImportAuditSummary(payload = {}) {
  ...
  return brainStores.reduce((summary, store) => {
...
function knowledgeBrainReviewQueueActionSummary(items = []) {
  ...
  for (const item of items) {
    for (const reason of item.reasons || []) {
      ...
      if (!current.targetIds.includes(targetKey)) current.targetIds.push(targetKey);
...
function knowledgePanelSearchResults({ query, documents = [], chunks = [], sources = [], units = [], policyRules = [], scenarios = [], evalCases = [], japaneseRealEstateRecords = [], calculationRuns = [], limit = 12 } = {}) {
  const keyword = `${query || ""}`.trim().toLowerCase();
  if (!keyword) return [];
...
async function searchKnowledgeForPanel(query) {
  ...
  const [documents, chunks, sources, units, policyRules, scenarios, evalCases, calculationRuns, ...jreRecordGroups] = await Promise.all([
