import { normalizeWorkflowProtocol, workflowExternalDisclosureLines, workflowLifecycleSteps, workflowToolCallChecklist, workflowWorkboardCards, workflowWorkboardExecutionMode, workflowWorkboardHandoffs, workflowWorkboardSummary } from "./taskEngine.mjs";
import { isQuotaExhaustionMessage } from "./modelGateway.mjs";
...
function formatWorkflowRecordMarkdown(record, lang = "zh") {
  ...
  const lines = [
    ...
    ...(workboardCards.length
      ? [
        `${label("摘要", "サマリー", "Summary")}: ${label("可执行", "実行可", "Ready")} ${workboardSummary.ready} · ${label("阻塞", "ブロック", "Blocked")} ${workboardSummary.blocked} · ${label("失败", "失敗", "Failed")} ${workboardSummary.failed} · ${label("进度", "進捗", "Progress")} ${workboardSummary.completionRate}% · ${label("交接", "引き渡し", "Handoffs")} ${workboardSummary.handoffReady}/${workboardSummary.handoffBlocked} · ${label("下一步", "次の一手", "Next action")}: ${workboardSummary.nextAction}`,
        "",
        "| Agent | Status | Dependency | Execution | Task | Output | Handoff |",
        "| --- | --- | --- | --- | --- | --- | --- |",
        ...workboardCards.map(card => {
          const execution = workflowWorkboardExecutionMode(item, card, lang);
          return `| ${card.member || "-"} · ${card.title || "-"} | ${card.status || "-"} | ${card.dependencyState || "-"}${card.blockedBy?.length ? ` (${card.blockedBy.join(" / ")})` : ""} | ${execution.kind}${execution.blockers?.length ? ` (${execution.blockers.map(blocker => `${blocker.id}:${blocker.status}`).join(" / ")})` : ""} | ${truncateText(card.task, 200).replace(/\n/g, " ")} | ${truncateText(card.output, 160).replace(/\n/g, " ")} | ${card.handoffTo || "-"} |`;
        }),
        "",
...
function buildWorkflowArtifactRevisionPrompt(record, artifactIndex = 0, lang = "zh") {
  ...
  const workboardCards = workflowWorkboardCards(item, lang);
  const handoffBlock = workflowWorkboardHandoffs(workboardCards)
    .slice(0, 8)
    .map(handoff => `- ${handoff.from || "-"} -> ${handoff.to || "-"} · ${handoff.status || "-"}${handoff.output ? ` · ${summarizeLine(handoff.output, 180)}` : ""}${handoff.blockedBy?.length ? ` · blocked by ${handoff.blockedBy.join(" / ")}` : ""}`)
    .join("\n");
  const dependencyBlock = workboardCards
    .flatMap(card => workboardDependencyEvidence(card, workboardCards)
      .map(evidence => `- ${card.member || card.id || "-"} · ${card.title || "-"} depends on ${evidence.member || evidence.dependency || "-"}${evidence.title ? ` · ${evidence.title}` : ""} · ${evidence.status || "unknown"}${evidence.output ? ` · ${summarizeLine(evidence.output, 180)}` : ""}${evidence.blocked ? " · blocked" : ""}`))
    .slice(0, 12)
    .join("\n");
  const memberBlock = item.results
    .slice(0, 6)
  ...
  return [
    ...
    memberBlock || "-",
    "",
    `${label("Workboard 交接证据", "Workboard 引き渡しエビデンス", "Workboard handoff evidence")}:`,
    handoffBlock || "-",
    "",
    `${label("Workboard 依赖证据", "Workboard 依存エビデンス", "Workboard dependency evidence")}:`,
    dependencyBlock || "-",
    "",
...
function buildWorkflowEventRecoveryPrompt(record, actionIndex = 0, lang = "zh") {
  ...
  const completed = item.results
    ...
    .map(result => `- ${result.member} · ${result.title}: ${result.summary || summarizeLine(result.text)}`)
    .join("\n");
  const gateInstruction = action?.type === "resolve_local_only_gate"
    ? label("闸门处理：不要绕过 Local-only。请先把任务改写为本地可执行步骤，或明确列出需要用户关闭 Local-only 后才能继续的外发动作；确认后只恢复被阻塞的卡片。", "ゲート処理：Local-only を迂回しないでください。まずタスクをローカル実行可能な手順へ書き換えるか、Local-only 解除後に必要な外部送信操作を明記し、確認後にブロックされたカードだけを復旧してください。", "Gate handling: do not bypass Local-only. First rewrite the task into local-executable steps, or list the external actions that require the user to disable Local-only; after confirmation, recover only the blocked card.")
    : action?.type === "resolve_execution_gate"
      ? label("闸门处理：先完成外发、管理员或工具权限确认，再继续该事件对应的卡片；未确认前不要调用外部工具。", "ゲート処理：外部送信、管理者、またはツール権限確認を完了してから、このイベントに対応するカードを続行してください。未確認のまま外部ツールを呼び出さないでください。", "Gate handling: complete external-send, admin, or tool permission confirmation before continuing the card for this event; do not call external tools before confirmation.")
      : "";
  return [
    ...
    gateInstruction ? `${label("闸门处理", "ゲート処理", "Gate handling")}: ${gateInstruction}` : "",
    "",
    `${label("已完成成果", "完了済み成果", "Completed outputs")}:`,
    completed || "-",
    "",
...
function buildWorkflowKnowledgePayload(record, lang = "zh", { memoryStatus = "approved", documentStatus = "approved" } = {}) {
  ...
  const workboardCards = workflowWorkboardCards(item, lang);
  const workboardHandoffs = workflowWorkboardHandoffs(workboardCards);
  const workboardDependencies = workboardCards
    .flatMap(card => workboardDependencyEvidence(card, workboardCards).map(evidence => ({
      cardId:card.id,
      card:`${card.member || card.id || ""}${card.title ? ` · ${card.title}` : ""}`.trim(),
      ...evidence,
    })))
    .slice(0, 24);
  const safeMemoryStatus = ["approved", "candidate"].includes(memoryStatus) ? memoryStatus : "candidate";
  ...
  return {
    ...
    memory:{
      ...
      content:[
        ...
        workboardDependencies.length ? `workboardDependencies: ${workboardDependencies.map(item => `${item.card || "-"} depends on ${item.member || item.dependency || "-"} · ${item.status || "unknown"}${item.output ? ` · ${item.output}` : ""}${item.blocked ? " · blocked" : ""}`).join(" / ")}` : "",
        "",
        `${label("任务", "タスク", "Task")}:`,
        item.task || "-",
        "",
...
function workboardDependencyEvidence(card = {}, cards = []) {
  ...
  return (Array.isArray(card.dependencies) ? card.dependencies : []).map(dependency => {
    ...
    workboard:workboardCards.length
      ? {
        ...
        cards:workboardCards.slice(0, 8).map(card => ({
          ...
          executionMode:workflowWorkboardExecutionMode(item, card, lang),
          dependencyEvidence:workboardDependencyEvidence(card, workboardCards),
...
function workflowRecordNeedsAttention(record = {}) {
  ...
  if (item.events.some(event => isQuotaExhaustionMessage(`${event.status || ""} ${event.detail || ""}`))) return true;
...
function workflowEventRecoveryActions(record = {}, lang = "zh") {
  ...
  for (const event of item.events) {
    ...
    const quotaExhausted = isQuotaExhaustionMessage(`${event.status || ""} ${event.detail || ""} ${event.error || ""} ${event.message || ""}`);
    if (quotaExhausted) {
      actions.push({
        type:"stop_quota_exhausted_automation",
        member:event.member || "",
        label:label("额度用尽，停止自动化", "クォータ枯渇のため自動化停止", "Quota exhausted: stop automation"),
        detail:event.detail || event.error || event.message || label("检测到额度或频率限制，停止自动化，避免重复唤醒和无效重试。", "クォータまたはレート制限を検出したため、自動化を停止し、無効な再試行を避けます。", "Quota or rate limit detected; stop automation to avoid repeated wakeups and ineffective retries."),
      });
      continue;
    }
