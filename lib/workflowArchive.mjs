import { normalizeWorkflowProtocol } from "./taskEngine.mjs";
...
function buildWorkflowKnowledgePayload(record, lang = "zh") {
  const item = normalizeWorkflowRecord(record);
  const label = (zh, ja, en) => lang === "ja" ? ja : lang === "en" ? en : zh;
  ...
  return {
    ...
    plan:item.plan?.steps?.length
      ? {
        ...
        steps:item.plan.steps.slice(0, 8).map(step => `${step.order}. ${step.member} · ${step.title}`),
      }
      : null,
