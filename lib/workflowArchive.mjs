import { normalizeWorkflowProtocol, workflowExternalDisclosureLines, workflowToolCallChecklist } from "./taskEngine.mjs";
  ...
  const lines = [
    ...
    ...(item.modelUsage?.models?.length
      ? [
        ...
        "",
        ...item.modelUsage.models.map(model => `- ${model.modelKey}${model.provider ? ` · ${model.provider}` : ""}${model.external ? "" : " · local/no external provider"}`),
      ]
      : ["-"]),
    "",
...
function formatWorkflowAuditMarkdown(record, lang = "zh") {
  ...
  return [
    ...
    ...(item.modelUsage?.models?.length
      ? item.modelUsage.models.map(model => `- ${model.modelKey || "-"} · ${model.provider || label("未记录提供商", "提供元未記録", "provider not recorded")} · ${model.external ? label("外发", "外部", "external") : label("本地/无外部提供商", "ローカル/外部提供元なし", "local/no external provider")}`)
      : []),
    "",
...
function buildWorkflowRecoveryPrompt(record, lang = "zh") {
  ...
  const failedMembers = item.members
    ...
    .map(member => `- ${member.name} · ${member.title} · ${member.model}`)
    .join("\n");
  return [
    ...
    failedMembers || "-",
    "",
    label("请只重试失败或缺失部分，复用已完成成果，最后由 ARIA 整合新版产物。", "完了済み成果を再利用し、失敗または欠落部分だけを再試行し、最後に ARIA が新版成果物として統合してください。", "Retry only the failed or missing parts, reuse completed outputs, and have ARIA integrate the updated artifact."),
  ].join("\n").trim();
}
  ...
  return {
    ...
    modelUsage:item.modelUsage?.models?.length
      ? {
        ...
        lines:item.modelUsage.models.map(model => `${model.modelKey}${model.provider ? ` · ${model.provider}` : ""}`),
...
function workflowRecordNeedsAttention(record = {}) {
  const item = normalizeWorkflowRecord(record);
  if (["failed", "waiting_confirmation"].includes(item.status)) return true;
