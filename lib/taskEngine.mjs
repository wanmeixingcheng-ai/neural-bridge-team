...
export function workflowPermissionChecklist(workflow = {}, lang = "zh") {
  ...
  const entries = [
    {
      ...
      status:modelUsage.external ? "needs_disclosure" : "ok",
      detail:modelUsage.external
    ...
    },
    {
      ...
      status:hasCodex ? "admin_required" : "ok",
      detail:hasCodex
    ...
    },
    {
      ...
      status:hasDeploy ? "admin_required" : "ok",
      detail:hasDeploy
  ...
  return {
    blocked:entries.some(entry => entry.status === "needs_confirmation" || entry.status === "admin_required"),
    entries,
  };
}
  ...
  const entries = [
    {
      ...
      status:modelUsage.models?.length ? "recorded" : "pending",
      detail:modelUsage.external ? label("调用外部模型提供商。", "外部モデル提供元を呼び出します。", "Calls external model providers.") : label("未记录外部模型调用。", "外部モデル呼び出しは未記録です。", "No external model call recorded."),
    },
    {
      id:"knowledge",
    ...
    },
    {
      ...
      permission:"external",
      status:hasWeb ? "needs_confirmation" : "not_needed",
      detail:hasWeb ? label("读取网页会把 URL 发送到服务端代理。", "Web読取は URL をサーバープロキシへ送信します。", "Fetching pages sends URLs to the server proxy.") : label("未检测到网页读取需求。", "Web読取の必要は検出されていません。", "No web fetch need detected."),
    },
    {
      id:"codex-dispatch",
      name:label("Codex/GitHub 投递", "Codex/GitHub 投递", "Codex/GitHub dispatch"),
      permission:"admin",
      status:hasCodex ? "needs_admin" : "not_needed",
      detail:hasCodex ? label("需要管理员 token 和投递确认。", "管理者 token と投递確認が必要です。", "Requires admin token and dispatch confirmation.") : label("未分派 Codex 执行。", "Codex 実行は割当られていません。", "No Codex execution assigned."),
    },
    {
      id:"vercel-deploy",
      name:label("Vercel 部署", "Vercel デプロイ", "Vercel deploy"),
      permission:"admin",
      status:hasDeploy ? "needs_admin" : "not_needed",
      detail:hasDeploy ? label("真正部署前必须确认生产变更。", "実デプロイ前に本番変更確認が必要です。", "Production changes must be confirmed before real deploy.") : label("未检测到部署动作。", "デプロイ操作は検出されていません。", "No deploy action detected."),
    },
  ];
  return {
    needsAttention:entries.some(entry => ["needs_confirmation", "needs_admin"].includes(entry.status)),
    entries,
  };
}
