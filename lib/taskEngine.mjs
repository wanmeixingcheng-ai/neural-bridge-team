...
export function workflowWorkboardExecutionMode(workflow = {}, card = {}, lang = "zh") {
  const label = (zh, ja, en) => lang === "ja" ? ja : lang === "en" ? en : zh;
  if (card.dependencyState === "blocked") {
    return {
      kind:"blocked_dependency",
      label:label("等待上游依赖", "上流依存待ち", "Waiting for dependency"),
      canStart:false,
      blockers:[{ id:"workboard-dependency", name:label("Workboard 依赖", "Workboard 依存", "Workboard dependency"), status:"blocked", permission:"workflow", detail:(card.blockedBy || []).join(" / ") }],
      tools:[],
      reason:label("上游卡片尚未完成，不能把这张卡标记为真实执行。", "上流カードが未完了のため、このカードを実行済み扱いにできません。", "Upstream cards are not complete, so this card cannot be treated as executed."),
    };
  }

  const taskText = `${workflow.task || workflow.title || ""}\n${card.task || ""}\n${card.input || ""}\n${card.output || ""}\n${card.acceptanceCriteria || ""}`;
  const cardModel = `${card.model || ""}`.toLowerCase();
  const relevantToolIds = [
    cardModel === "codex" ? "codex-dispatch" : "",
    taskMatches(taskText, ["http://", "https://", "网页", "网站", "url", "web", "fetch", "抓取", "读取链接"]) ? "web-fetch" : "",
    taskMatches(taskText, ["部署", "上线", "vercel", "deploy", "publish"]) ? "vercel-deploy" : "",
  ].filter(Boolean);
  const toolCalls = workflowToolCallChecklist(workflow, lang).entries
    .filter(entry => relevantToolIds.includes(entry.id));
  const blockers = toolCalls
  ...
  if (blockers.length) {
    return {
      ...
      label:label("等待权限/配置", "権限/設定待ち", "Waiting for permission/config"),
      canStart:false,
      blockers,
      tools:toolCalls,
      reason:label("该卡片需要真实工具或外部服务，但权限、配置或 Local-only 状态尚未满足。", "このカードは実ツールまたは外部サービスを必要としますが、権限、設定、または Local-only 状態が未解決です。", "This card needs a real tool or external service, but permission, config, or Local-only state is unresolved."),
    };
  }
  ...
  if (toolCalls.length) {
    return {
      kind:"tool_dispatch",
      label:label("真实工具投递", "実ツール投递", "Real tool dispatch"),
      canStart:true,
      blockers:[],
      tools:toolCalls,
      reason:label("该卡片必须通过工具队列或外部动作产生可验证结果。", "このカードはツールキューまたは外部アクションで検証可能な結果を生成する必要があります。", "This card must produce verifiable output through a tool queue or external action."),
    };
  }

  return {
    kind:"model_output",
    label:label("模型产出", "モデル出力", "Model output"),
    canStart:true,
    blockers:[],
    tools:[],
    reason:label("该卡片可以由成员模型生成交付物；若需要读写文件、部署或外部系统，必须升级为工具投递。", "このカードは担当モデルで成果物を生成できます。ファイル操作、デプロイ、外部システムが必要な場合はツール投递へ昇格してください。", "This card can be delivered by the member model; file writes, deploys, or external systems must be escalated to tool dispatch."),
  };
}
...
export function buildWorkboardCardActionPrompt(workflow = {}, card = {}, action = "continue", lang = "zh") {
  ...
  return [
    ...
    `${label("完成门槛", "完了ゲート", "Completion gate")}: ${packet.completionGate?.policy || "-"}`,
    `${label("所需证据", "必要証拠", "Required evidence")}: ${packet.completionGate?.requiredEvidence?.length ? packet.completionGate.requiredEvidence.join(" / ") : "-"}`,
...
export function buildWorkboardExecutionPacket(workflow = {}, card = {}, lang = "zh") {
  ...
  const toolCalls = workflowToolCallChecklist(workflow, lang);
  const cards = workflowWorkboardCards(workflow, lang);
  const execution = workflowWorkboardExecutionMode(workflow, card, lang);
  ...
  const blocked = !execution.canStart;
  const completionGate = {
    mayMarkComplete:!blocked && readiness.canExecute && execution.kind === "model_output",
    requiresArtifact:["model_output", "tool_dispatch"].includes(execution.kind),
    requiresToolEvidence:execution.kind === "tool_dispatch",
    requiredEvidence:execution.kind === "tool_dispatch"
      ? execution.tools.map(tool => tool.id || tool.name).filter(Boolean)
      : execution.kind === "model_output"
        ? [label("成员产物", "メンバー成果物", "member artifact")]
        : execution.kind === "blocked_permission"
          ? [...new Set([
              ...execution.blockers.map(blocker => blocker.id || blocker.name).filter(Boolean),
              ...requiredTools.map(tool => tool.id || tool.name).filter(Boolean),
            ])]
        : execution.blockers.map(blocker => blocker.id || blocker.name).filter(Boolean),
    policy:execution.kind === "tool_dispatch"
      ? label("工具派发卡片只有在记录真实工具结果后才能标记完成。", "ツール投递カードは実ツール結果を記録した後のみ完了にできます。", "Tool-dispatch cards can only be marked complete after real tool results are recorded.")
      : execution.kind === "model_output"
        ? label("模型卡片必须产出可交付内容；启动或回复本身不等于完成。", "モデルカードは引き渡し可能な成果物が必要です。起動や返信だけでは完了ではありません。", "Model cards must produce a handoff-ready artifact; starting or replying is not completion.")
        : label("阻塞卡片不能标记完成，必须先解决依赖或权限。", "ブロックされたカードは完了にできません。先に依存または権限を解決してください。", "Blocked cards cannot be marked complete until dependency or permission blockers are resolved."),
  };
  return {
    ...
    instructions:[
      ...
      `${label("执行类型", "実行種別", "Execution type")}: ${execution.kind} · ${execution.label}`,
      execution.reason ? `${label("执行说明", "実行説明", "Execution note")}: ${execution.reason}` : "",
      `${label("完成门槛", "完了ゲート", "Completion gate")}: ${completionGate.policy}`,
...
export function buildWorkboardExecutionPacketEvent(packet = {}, at = new Date().toISOString()) {
  ...
  const dependencyEvidence = Array.isArray(packet.card?.dependencyEvidence) ? packet.card.dependencyEvidence : [];
  const requiredEvidence = Array.isArray(packet.completionGate?.requiredEvidence) ? packet.completionGate.requiredEvidence : [];
  return {
    at,
    ...
    detail:[
      ...
      packet.execution?.kind ? `execution=${packet.execution.kind}` : "",
      packet.canExecute ? "can_execute=yes" : "can_execute=no",
      packet.completionGate ? `mark_complete=${packet.completionGate.mayMarkComplete ? "yes" : "no"}` : "",
      requiredEvidence.length ? `required_evidence=${requiredEvidence.join(" / ")}` : "",
      ...
      dependencyEvidence.length ? `deps=${dependencyEvidence.map(item => `${item.dependency || item.member}:${item.status || "unknown"}${item.blocked ? ":blocked" : ""}`).join(" / ")}` : "",
...
export function chooseWorkflowMembers(group, taskText) {
  ...
  if (taskMatches(taskText, ["codex 创建", "codex 建立", "codex 投递", "codex task", "codex任务", "github issue", "创建 issue", "建立 issue", "测试 issue", "任务投递", "投递任务"])) {
    const codexMember = members.find(member => `${member.model || ""}`.toLowerCase() === "codex" && member.id !== "aria");
    if (codexMember) return [codexMember];
  }
...
export function ensureExecutableWorkflowMembers(workers = [], allMembers = [], taskText = "") {
  ...
  if (taskMatches(taskText, ["codex 创建", "codex 建立", "codex 投递", "codex task", "codex任务", "github issue", "创建 issue", "建立 issue", "测试 issue", "任务投递", "投递任务"])) {
    const codexMember = (Array.isArray(allMembers) ? allMembers : [])
      .find(member => member?.id !== "aria" && `${member.model || ""}`.toLowerCase() === "codex");
    if (codexMember) return [codexMember];
  }
...
export function isWorkflowConfirmationCommand(text = "") {
  ...
  return [
    ...
    /confirm.*continue/.test(normalized) ||
    /continue.*workflow/.test(normalized);
}

function workflowMemberMatches(member = {}, candidate = {}) {
  const values = [candidate.id, candidate.name, candidate.title, candidate.member]
    .filter(Boolean)
    .map(value => `${value}`.trim().toLowerCase());
  return [member.id, member.name, member.title, member.member]
    .filter(Boolean)
    .some(value => values.includes(`${value}`.trim().toLowerCase()));
}

export function workflowCompletedResultsFromState(workflow = {}) {
  const members = Array.isArray(workflow.members) ? workflow.members : [];
  return members
    .filter(member => member.status === "complete" && (member.text || member.summary))
    .map(member => ({
      member:member.name || member.id || "",
      title:member.title || "",
      model:member.model || "",
      text:member.text || member.summary || "",
      summary:member.summary || summarizeForWorkflow(member.text || ""),
      executionEvidence:member.executionEvidence || null,
    }));
}

export function resolveWorkflowExecutionMembers(workflow = {}, allMembers = []) {
  const savedMembers = Array.isArray(workflow.members) ? workflow.members : [];
  const planSteps = Array.isArray(workflow.plan?.steps) ? workflow.plan.steps : [];
  const candidateMembers = savedMembers.length
    ? savedMembers.filter(member => member.status !== "complete" && member.status !== "skipped")
    : planSteps.map(step => ({
        id:step.memberId || "",
        name:step.member || "",
        title:step.title || "",
  ...
  return candidateMembers
    .map((saved, index) => {
      const base = (Array.isArray(allMembers) ? allMembers : []).find(member => workflowMemberMatches(member, saved)) || {};
      const step = planSteps.find(item => workflowMemberMatches(saved, {
        id:item.memberId,
        name:item.member,
        title:item.title,
      })) || planSteps[index] || {};
      return {
        ...base,
        ...saved,
        id:saved.id || base.id || step.memberId || `workflow-member-${index + 1}`,
        name:saved.name || base.name || step.member || saved.id || "",
        title:saved.title || base.title || step.title || "",
        model:saved.model || step.model || base.model || "",
        systemPrompt:base.systemPrompt || saved.systemPrompt || "",
        status:saved.status || "queued",
        task:saved.task || step.subtask || step.purpose || "",
      };
    })
    .filter(member => member.id !== "aria");
}
