import { emptyWorkflowState, normalizeWorkflowProtocol } from "./taskEngine.mjs";
...
function normalizeWorkflowState(state, lang = "zh") {
  ...
  return {
    ...
    trigger: state.trigger && typeof state.trigger === "object"
      ? {
        type: `${state.trigger.type || ""}`.slice(0, 80),
        automationId: `${state.trigger.automationId || ""}`.slice(0, 120),
        label: `${state.trigger.label || ""}`.slice(0, 160),
      }
      : null,
    ...
    plan: state.plan && typeof state.plan === "object"
      ? {
        ...
        protocol: normalizeWorkflowProtocol(state.plan.protocol || {}, { taskText:state.task || state.plan.task || "" }),
    ...
    modelUsage: state.modelUsage && typeof state.modelUsage === "object"
      ? {
        ...
        localOnlyMode: !!state.modelUsage.localOnlyMode,
    ...
    artifacts: Array.isArray(state.artifacts)
      ...
      : [],
    comments:normalizeWorkflowComments(state.comments),
