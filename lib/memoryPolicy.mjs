...
export function archiveMemoryConflictPatch(metadata = {}) {
  return {
    status:"archived",
    archived:true,
    metadata:{
      ...clearMemoryConflictMetadata(metadata),
      conflictArchivedAt:new Date().toISOString(),
    },
  };
}

export function memoryHasConflict(item = {}) {
  return Boolean(item?.metadata?.conflict);
}
